...
対応付けてもよい．\margin{Linux の Pthreads ライブラリは NPTL であり，
各スレッドに専用のカーネルタスク（\texttt{clone(2)}）を与える．一対一
モデルである（第5章）．}
...
情報を持つスレッドのデータ構造に到達する．\caution{不透明なので，2つの
識別子は \texttt{==} ではなく \texttt{pthread\_equal(t1, t2)} で比較する．
POSIX は \texttt{pthread\_t} が構造体であることを許している．}スレッドの生成とジョインには
2つの関数を用いる．
...
0 を，失敗したときはエラー番号を返す．\margin{生成とジョインの往復は，
現在の x86-64 Linux では \SI{10}{\micro\second} 程度である．スレッド
プール（第3章）はこれをワーカーごとに一度だけ払う．}
...
スレッドが生成される．\margin{既定のスタックは \texttt{RLIMIT\_STACK} の
ソフトリミットで，通常は \SI{8}{\mebi\byte} のアドレス空間である
（第3章）．最小値は \texttt{PTHREAD\_STACK\_MIN} で，x86-64 では
\SI{16}{\kibi\byte} である（\texttt{pthread\_attr\_setstacksize(3)}）．}属性オブジェクト自体も不透明であり，関数を通じてのみ
操作する．
\begin{lstlisting}[language=C, numbers=none]
...
\margin{\texttt{SCHED\_OTHER} は第7章の CFS である．
\texttt{SCHED\_FIFO} と \texttt{SCHED\_RR} はリアルタイムであり，優先度は
1--99，クォンタムは \SI{100}{\milli\second} である（\texttt{sched(7)}）．}
...
いる間ずっと資源を占有する．\margin{第2章のゾンビプロセスと同じ勘定である．
ワーカーを決してジョインしないサーバは，やがて新しいスレッドを生成でき
なくなる．}さらにメインスレッドは，子が終わる前に
...
できる．\tip{誰もジョインしないスレッドは，最初の文で
\texttt{pthread\_detach(pthread\_self())} を呼んで自分をデタッチする．}
\begin{lstlisting}[language=C, numbers=none]
...
\begin{enumerate}
  ...
  \item \textbf{コンパイラのフラグ．}プログラムは \texttt{-pthread} または
    ...
    \margin{GNU/Linux では \texttt{-pthread} は \texttt{\_REENTRANT} も
    定義する．glibc~2.34 以降，Pthreads の関数は \texttt{libc} の一部で
    ある．}
...
\end{lstlisting}
  ...
  \item \textbf{戻り値の検査．}Pthreads の関数は，\texttt{errno} ではなく
    戻り値で失敗を報告する．\caution{Pthreads は POSIX の例外である．
    システムコールのラッパーは $-1$ を返して \texttt{errno} を設定するので，
    \texttt{pthread\_create} の失敗後の \texttt{perror} は誤った原因を
    表示する．}成功なら 0，そうでなければエラー番号である．
...
ループがそれらをジョインする．各ワーカーは1行を表示する．\margin{行が混ざることはない．POSIX は stdio の
各呼び出しにストリームをロックさせる（\texttt{flockfile(3)}）ので，
\texttt{printf} にミューテックスは要らない．}ワーカーは何の
...
引数を通じて番号を読む．\margin{番号を値として \texttt{(void~*)(intptr\_t)i}
の形で渡し，キャストで戻す書き方もある．共有変数はなくなるが，この変換は
処理系定義である．}
\begin{lstlisting}[language=C, numbers=none]
...
\end{lstlisting}
このプログラムが $0$，$1$，$2$ を表示する保証はない．\tip{競合検出器を
使えば運に頼らずに済む．\texttt{-fsanitize=thread} を付けてビルドするか，
\texttt{valgrind --tool=helgrind} の下で実行する．}どの
...
受け取るようにすれば，競合はなくなる．\margin{生成元が待てない場合の通常の
構成は，スレッドごとに1つの引数構造体を \texttt{malloc} で確保し，
スレッド自身が解放するというものである．}
\begin{lstlisting}[language=C, numbers=none]
...
並びも起こりうる．\margin{3行の並び方は $3! = 6$ 通りあり，ある実行で順に
表示されても残りの5通りについては何も言えない．}配列は \texttt{main} のスタックフレームにあり，
...
ブロックする．\sidenote{空いているミューテックスのロックは，ユーザ空間の
アトミックなリードモディファイライト命令1つで済む．glibc が
\texttt{futex(2)} を通じてカーネルに入るのは，スレッドがブロックしなければ
ならないときと，他のスレッドを起こさなければならないときだけである．
したがって競合のないロックは数十ナノ秒で済み，競合するロックは
システムコールとコンテキストスイッチの分だけかかる．Drepper，
「Futexes Are Tricky」，2011．}違いは構文上のものだが，重要である．Birrell の
...
\margin{型は小さい．x86-64 の glibc では \texttt{pthread\_mutex\_t} が
\SI{40}{\byte}，\texttt{pthread\_cond\_t} が \SI{48}{\byte} なので，
保護する構造体ごとにロックを埋め込める．}
\begin{lstlisting}[language=C, numbers=none]
...
\begin{description}
  \item[\texttt{pthread\_mutex\_init}] は，最初に使う前にミューテックスを
    ...
    \margin{型も属性である．既定の型は所有者が再びロックするとデッド
    ロックし（第3章），\texttt{PTHREAD\_MUTEX\_ERRORCHECK} は代わりに
    \texttt{EDEADLK} を返し，\texttt{PTHREAD\_MUTEX\_RECURSIVE} は回数を
    数える．}
...
\begin{example}
  ...
  空いているときだけ補助的な統計情報を更新する．\margin{\texttt{trylock}
  をループの中で呼ぶとスピンロックになり，他のスレッドがミューテックスを
  保持している間 CPU を浪費する．第10章がスピンとブロックを比較する．}
  \begin{lstlisting}[language=C, numbers=none]
...
\begin{itemize}
  ...
  \item \textbf{ミューテックスは大域的な順序でロックする．}すべての
    ...
    起こりえない．\index{ろっくじゅんじょ@ロック順序}\tip{ミューテックスに番号を付け，
    常に番号の小さい方から順にロックする．より小さい番号のものが必要に
    なったスレッドは，保持しているものを解放してやり直す．}
...
必須である．\sidenote{POSIX.1 \cite{posix2017} の根拠はこの許容を次のように
説明している．wait から無駄に戻ることが決してないことを要求すると，
マルチプロセッサでは条件変数が遅くなり，実装が
\texttt{pthread\_cond\_signal} を broadcast として実現することも許され
なくなる．どのみち述語は検査しなければならないので，ループに費用はなく，
余分な通知に対してプログラムが強くなる．}
\begin{lstlisting}[language=C, numbers=none]
...
を与える．\margin{プロセス間で共有するには，条件変数とそのミューテックスの
両方に \texttt{PTHREAD\_PROCESS\_SHARED} を設定し，どちらも共有メモリに
置かなければならない（第9章）．}
...
\begin{itemize}
  ...
  \item \textbf{迷ったら broadcast する．}signal で足りるところで
    ...
    \margin{待っているスレッドが $n$ 個あると，broadcast は $n$ 回の起床と
    $n$ 回のミューテックス獲得を要し，そのうち $n-1$ 回は無駄である．
    第3章の「thundering herd」である．}
...
添字 \texttt{rem}，そしてバッファ内の値の個数 \texttt{num} である．\margin{1つのスロットを
空けておけば \texttt{num} は不要になる．そのときバッファが空なのは
\texttt{add == rem} のとき，満杯なのは
\texttt{(add + 1) \% BUF\_SIZE == rem} のときである．}どちらの
...
生産者は値 $1,\dots,\texttt{NUM\_ITEMS}$ を挿入する．\tip{\texttt{BUF\_SIZE}
が2のべきなら，\texttt{(add + 1) \% BUF\_SIZE} は
\texttt{(add + 1) \& (BUF\_SIZE - 1)} と書ける．除算1つが命令1つになる．}
\begin{lstlisting}[language=C, numbers=none]
void *producer(void *param) {
  int i;
...
消費者は対称である．\margin{各スレッドは自分のスタックに自分の \texttt{i} と
\texttt{n} を持つ（第3章）．共有されているのは \texttt{buffer}，
\texttt{add}，\texttt{rem}，\texttt{num} だけである．}
\begin{lstlisting}[language=C, numbers=none]
void *consumer(void *param) {
  int i, n;
...
\begin{itemize}
  ...
  \item 各スレッドはアンロックの後に signal するので，起こされたスレッドは
    \texttt{m} を待たずに済む．\margin{\texttt{printf} がクリティカル
    セクションの外にあるのも同じ理由である．端末への書き込みはブロックし
    うるし，\texttt{m} を保持したままブロックすればもう一方が止まる．}誰も待っていないときの signal は何もしないが，
    これは無害である．スレッドは待つ前に述語を検査するので，すでに起きた
    変化を待つことは決してないからである．
  \item 生産者は \texttt{c\_prod} だけで，消費者は \texttt{c\_cons} だけで
    待つので，通知は必ず，それを利用できる種類のスレッドに届く．%
    \caution{両方に1つの条件変数を使うと，消費者しか進めないときに
    \texttt{signal} が生産者を起こしうる．そのような設計には broadcast が
    必要である．}
