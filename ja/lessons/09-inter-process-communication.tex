...
カーネルは，\margin{x86-64 では何もしないシステムコール
でも \SI{100}{\nano\second} の桁の時間がかかり（lmbench の
\texttt{lat\_syscall}），Meltdown 対策の KPTI があるとその倍になりうる．}%
通信の確立にも，個々の IPC 操作のすべてにも関与する．送信は，
...
を2要素の配列 \texttt{fd} に格納する．\margin{Linux のパイプは
\SI{64}{\kilo\byte} を保持し，\texttt{PIPE\_BUF} $=\SI{4}{\kilo\byte}$
以下の書き込みはアトミックである（\texttt{pipe(7)}）．}典型的な用途は，シェルが
...
\margin{Linux の既定値は，System~V のメッセージ1つが \SI{8}{\kilo\byte}，
1つのキューが \SI{16}{\kilo\byte}，POSIX のキューが10メッセージである
...
\margin{対応付けが動かすのはデータではなくページテーブルエントリである
（第8章）．\SI{2}{\mega\byte} のヒュージページなら，1つのエントリで512個の
基本ページを覆う（\texttt{SHM\_HUGETLB}）．}
...
ashmem を加えている．\sidenote{ashmem の領域は\emph{アンピン}できる．内容は捨てて
よいものとなり，メモリ逼迫時にカーネルはプロセスを終了させる代わりに領域を
回収する．再びピンすると破棄されたかが分かる．Android はこの用途を
\texttt{memfd\_create} へ移しつつある．}
...
下回る．\tip{例の定数で $n^{*} \le 1$ を解くと $S \approx
\SI{600}{\kilo\byte}$ となる．これを超えれば1回の転送だけで対応付けの元が
取れる．}
...
\begin{example}
  ...
  0.26$ である．\margin{1キロバイト当たり \SI{0.1}{\micro\second} は
  \SI{10}{\giga\byte\per\second} であり，1つのコアが主記憶に対して持つ
  コピー帯域の桁である．}ブロックする待機と起床（$t_{\mathrm{sync}} =
...
\begin{description}
  \item[\texttt{shmget(key, size, flags)}] は \texttt{key} に結び付いた
    ...
    呼び出しは失敗する）．セグメントの識別子を返す．\margin{\texttt{size} は
    ページ単位に切り上げられるので，最小のセグメントは \SI{4}{\kilo\byte} の
    1ページである（\texttt{shmget(2)}）．}
  \item[\texttt{ftok(pathname, proj\_id)}] は，既存のファイルの同一性
    ...
    セグメントを見つけられる．\caution{キーが保持するのは \texttt{proj\_id}
    の1バイト，i ノード番号の下位 16 ビット，デバイス番号の下位 8 ビットだけ
    なので，別のファイルどうしが同じキーになりうる（\texttt{ftok(3)}）．}
...
\texttt{munmap} はそれを切り離す．\caution{\texttt{MAP\_PRIVATE} で対応付ける
と，代わりにコピーオンライトになる．最初の書き込みでそのプロセス専用のページ
ができ，何も共有されない（\texttt{mmap(2)}）．}\texttt{close} は，確立済みの対応付けに
...
\end{lstlisting}
  ...
  する．\sidenote{プロセスはクリティカルセクションの中で死ぬことがあり，その
  ときミューテックスは永久にロックされたままになる．属性オブジェクトに
  \texttt{PTHREAD\_MUTEX\_ROBUST} を設定したミューテックスは，代わりに次に
  ロックするプロセスへ \texttt{EOWNERDEAD} を返す．そのプロセスは共有状態を
  修復して \texttt{pthread\_mutex\_consistent} を呼ぶ．さもなければミュー
  テックスは使えなくなる（\texttt{ENOTRECOVERABLE}）．}\allowbreak
  \texttt{IPC\_EXCL} フラグは，セグメントがすでに存在する場合に
...
である．\margin{POSIX の名前付きセマフォ（\texttt{sem\_open}）は作りからして
プロセス共有である．名前のないものは \texttt{pshared} を設定し，領域の中に
置かなければならない（\texttt{sem\_overview(7)}）．}値 0 と 1 で使えば
ミューテックスのように振る舞う．値が 0 であるのを
...
共有メモリに限定する．\tip{\texttt{nattch} の欄は，そのセグメントに現在接続
しているプロセスの数である．どのプログラムも開き直さないのに \texttt{nattch}
が 0 のセグメントはリークである．}\texttt{ipcrm} コマンドは機構を取り除く．
...
\begin{description}
  ...
  \item[多数の小さなセグメント] 通信するプロセスの組ごとに専用のセグメントを
    ...
    あらかじめ作り，空いているものの識別子をキューに保持する．\margin{プールを
    あらかじめ確保してその要素を配るのは，第3章のボス・ワーカー型のスレッド
    プールと同じ論法をセグメントに適用したものである．}プロセスはプール
...
\end{lstlisting}
  ...
  戻して signal する．\margin{これは第4章の生産者・消費者の有界バッファで
  スロットが1つの場合であり，スロットはセグメントのペイロード領域にあたる．}%
  \SI{5}{\mega\byte} の転送は $\lceil 5/2 \rceil = 3$
  ラウンドを要する:
  \begin{center}
    \small
    \begin{tabular}{@{}cllcc@{}}
