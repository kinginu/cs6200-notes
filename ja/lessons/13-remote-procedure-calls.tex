...
\margin{光ファイバ中の光の速さはおよそ
\SI{200}{\kilo\meter\per\milli\second} であり，\SI{5000}{\kilo\meter}
離れたマシンへの呼び出しは，両端のマシンがどれほど速くても
\SI{50}{\milli\second} 未満では戻らない．}
...
になっている．\sidenote{彼らの測定は，\SI{2.94}{\mega\bit\per\second} の
Ethernet で結ばれた2台の Dorado ワークステーションの間で行われ，オーバーヘッドの
規模を示している．引数も結果もない呼び出しの所要時間は中央値で
\SI{1097}{\micro\second}，そのうち転送が \SI{131}{\micro\second} であり，
同じ呼び出しをローカルに行うと \SI{9}{\micro\second} であった
\cite{birrell1984}．}
...
\begin{description}
  ...
  \item[4 受信] サーバの RPC ランタイムがメッセージを受信し，サーバスタブへ
    渡す．アクセス制御の検査はこの時点で行われる．\caution{Sun RPC の既定の
    資格情報 \texttt{AUTH\_SYS} は，クライアントが自称するユーザ ID と
    グループ ID にすぎない．本当の認証には \texttt{RPCSEC\_GSS} が要る
    \cite{rfc2203}．}
...
\begin{definition}[アンマーシャリング]
  ...
  を作る．\caution{マーシャリングと直列化はここでは同義である．区別する場合，
  直列化はバイト表現だけを指し，マーシャリングは受信側が値を再構成するのに
  必要なもの---Java ならクラスのコードベース---も含む．}
\end{definition}
...
\begin{example}
  ...
  \margin{データは行きと帰りで少なくとも2度---送信バッファへ，受信バッファ
  から---複製される．第9章の共有メモリは，まさにこの複製を避けるためにある．}
\end{example}
...
再送できるが，タイムアウトと再送は何も保証しない．\sidenote{応答が届くまで
再送すれば\emph{少なくとも1回}の意味論に，最初の試みで諦めれば\emph{高々1回}
の意味論になる．どちらもちょうど1回ではない．サーバが最近の要求識別子と応答の
対を保持し，再送に対しては手続きを実行し直さずその記録から答えれば，ちょうど1
回に近づく．NFS のサーバはこれを重複要求キャッシュと呼ぶ．}%
サーバが停止している間は再送
...
一意に識別する．\margin{プログラム番号
\texttt{0x00000001}--\texttt{0x1fffffff} は IANA が割り当て---NFS は
\texttt{100003} である---，\texttt{0x20000000}--\texttt{0x3fffffff} は各
サイトの管理者に委ねられる．上の \texttt{0x20000123} はこの範囲にある
\cite{rfc5531}．}\texttt{values} に3つの整数 7，$-2$，300 を設定して
...
（\cref{sec:l13-sun-binding}）．\tip{生成される名前には規則がある．手続き名を
小文字にしてバージョン番号を続け---\texttt{sum\_proc\_1}---，開発者が書く
手続きにはさらに \texttt{\_svc} が付く．}それぞれを，生成されたファイルとともに
コンパイル
...
プログラム \texttt{100003} として現れる．\margin{NFS は登録されたポート 2049
でも待ち受ける．NFSv4 のクライアントはバインディングのプロトコルを使わないこと
になっている \cite{rfc8881}．}最後の行が \texttt{sum} サーバで
...
クライアントはそれをすべてのスタブに渡す．\margin{サーバ自身のポートは動的に
割り当てられ，再起動のたびに変わるので，ファイアウォールであらかじめ開けておく
ことができない．また \texttt{rpcbind} は誰の問い合わせにも答えるので，露出した
デーモンはサービス拒否攻撃の増幅に悪用されてきた．}%
ハンドルはサーバのアドレスのほか，
...
再送する．\margin{UDP では RPC メッセージが1つのデータグラム---ペイロードは
最大 \SI{65507}{\byte}---に収まらなければならない．glibc のランタイムは
呼び出しがタイムアウトするまで \SI{5}{\second} ごとに再送する．}
...
山括弧で与え，長さのフィールドと要素へのポインタを持つ C の構造体になる．%
\margin{最大長は強制される．生成されたルーチンはそれより長い配列を拒むので，
受信側をあふれさせる代わりに呼び出しが失敗する．}この
...
\cite{rfc4506}．\caution{循環のある構造はこの符号化を破る．スタブがポインタを
永久にたどってしまうからである．Java のオブジェクト直列化は，各オブジェクトに
ハンドルを与え，2度目には後方参照を書く．}
...
\begin{description}
  \item[トランスポートヘッダ] TCP や UDP などのトランスポートプロトコルと，
    その下位のプロトコルのヘッダ．\margin{バイトストリームの上では，各メッセージ
    の前に \SI{4}{\byte} のレコードマークが置かれる．最上位ビットが最終
    フラグメントの印，残る 31 ビットが長さである \cite{rfc5531}．}
...
\begin{itemize}
  \item すべての項目は \SI{4}{\byte} の倍数で符号化される．短いデータはゼロ
    バイトで詰められる．\tip{$n$ バイトの項目は通信路上で
    $4\lceil n/4\rceil$ バイトを占め，可変長の配列や文字列はさらに
    \SI{4}{\byte} の長さフィールドを加える．}
...
下のクライアントのコードがそのサービスを呼び出す．\margin{RMI レジストリ自体も
遠隔オブジェクトであり，\texttt{LocateRegistry.getRegistry} は，別のポートを
指定しない限り TCP のポート 1099 にあるレジストリのスタブを返す．}
...
オブジェクトとともに直列化される．\caution{複製は呼び出しのたびに作られ，サーバ
が複製に加えた変更はクライアントからは見えない．\cref{sec:l13-pointers} の
直列化されたポインタと同じ落とし穴である．}%
遠隔オブジェクトはスタブとして参照渡しされる．
...
ある．\margin{gRPC は同じ4つの選択を別の形で行う．IDL と符号化には Protocol
Buffers を使い，レジストリの代わりに DNS とロードバランサを置き，すべての
呼び出しがステータスコードを返す．}
