...
\begin{tikzpicture}[x=1mm, y=1mm, font=\footnotesize\sffamily,
  ...
  num/.style={draw, circle, fill=white, inner sep=0pt, minimum size=3.4mm,
    font=\tiny\sffamily\bfseries, text=ln-accent},
  ...
  \draw[->] (chk) -- node[lab, above, pos=0.62] {\iflangja{いいえ：パイプ}{no: pipe}}
    node[num, below=0.6mm, pos=0.62] {3} (h2);
  % disk
  ...
  \draw[<->] (h2.east) -- node[lab, above] {\texttt{mmap}} (disk.west);
  % completion
  ...
  \node[lab, text=ln-caution, anchor=south] at (48,-19.6)
    {\iflangja{完了をパイプで通知（イベント）}{completion reported through the pipe (an event)}};
