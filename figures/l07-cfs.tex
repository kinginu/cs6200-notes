...
\begin{tikzpicture}[x=1mm, y=1mm, font=\footnotesize\sffamily,
  ...
  \draw[->, thick, ln-accent] (n9.west) -- ++(-9,0) node[left, lab, text=ln-accent]
    {\iflangja{最左ノード：\\vruntime 最小\\→ 次に実行}{leftmost node:\\smallest vruntime\\$\to$ runs next}};
  % a preempted task is reinserted
  \node[bn, dashed, fill=white, text=black, draw=black] (p) at (98,-8) {27};
  \node[lab, anchor=south] at (98,-3.5) {\iflangja{プリエンプト\\されたタスク}{preempted\\task}};
  \node[draw, dashed, circle, minimum size=7mm, inner sep=0pt] (ins) at (53,-38) {};
  \draw[dashed] (n30) -- (ins);
  \draw[->, thick, densely dashed] (p.south) .. controls (98,-44) and (70,-44) .. (ins.east)
    node[lab, pos=0.45, below] {\iflangja{挿入}{insert}};
  \node[lab, text=ln-muted, anchor=west] at (0,-44) {\iflangja{節点の値：仮想実行時間（ms）}{node values: virtual runtime (ms)}};
\end{tikzpicture}
