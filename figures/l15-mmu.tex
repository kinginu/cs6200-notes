...
% current mapping complete in hardware; the others fault, and the DSM either
% fetches the page or performs consistency operations before the access is
% repeated.
...
\begin{tikzpicture}[x=1mm, y=1mm, font=\scriptsize\sffamily,
  ...
  \node[box] (fetch) at (53,10) {\iflangja{所有者にページを要求，\\読み出し専用でマップ}{request the page from its\\owner; map it read-only}};
  \node[box, text width=32mm] (coord) at (93,10) {\iflangja{一貫性操作：他のレプリカを\\無効化，読み書き可でマップ}{consistency operations:\\invalidate other replicas;\\map it read-write}};
  \draw[->] (acc) -- (mmu);
  ...
  \node[lab, anchor=east, text=ln-muted, align=right] at (52.2,20) {\iflangja{ローカルに\\ない}{no local\\copy}};
