...
RPC or RMI across machines (Lesson~13).\margin{A tier boundary is also a
cost boundary: the same call costs nanoseconds within one process, a few
microseconds through local IPC (Lesson~9), and about
\SI{0.5}{\milli\second} by RPC to another machine in the
datacenter---Dean's ``numbers everyone should know''.}

\begin{figure}[htbp]
  \centering
  \input{figures/l16-tiers}
...
(Lesson~14).\margin{Which node holds which data is usually decided by
hashing a key---a user, a session, a file name.  Consistent hashing is the
common choice: adding the $N$th node then moves only about $1/N$ of the
keys.}  To increase the capacity, nodes are simply added.
...
URL, to determine which nodes can process it.\margin{A front end that only
forwards connections needs to look no further than the transport headers; one
that chooses a node by URL or cookie must terminate the connection and parse
the request itself.}  The management of the service becomes more complex as
well.

\subsection{Scaling and management}

The two architectures differ most when the load grows.\margin{Scale-out and
scale-up are also called horizontal and vertical scaling.  Either raises the
throughput of the service; neither shortens the processing of a single
request.}  To scale a homogeneous architecture, it suffices to add nodes,
together with the processes, machines and access to storage that they need.
Every new node takes its share of every type of request, and no knowledge of which requests
...
are idle.\margin{The same imbalance arises inside one machine in Lesson~7,
where load balancing migrates threads between per-CPU runqueues.
Specialized nodes cannot take over each other's work, because they lack the
data.}  \Cref{tab:l16-arch} summarizes the comparison.
...
\begin{example}
  ...
  = 6 + 4 = 10$ seconds of processing per second, the capacity of ten
  nodes.\tip{This is Little's law of Lesson~6: the number of nodes busy at
  once is the arrival rate times the service time, so the load in nodes is
  $\sum_i \lambda_i t_i$ whatever the mix of requests.}
...
Web Services (AWS).\caution{Amazon's own histories trace AWS not to idle
holiday capacity but to the shared platform it had built for its own
teams.}  Its Elastic Compute Cloud (EC2), launched in 2006, rents
...
configured; with rented resources it required only calls to an
API.\margin{EC2 opened as a limited public beta in August 2006 and became
generally available in October 2008.}
...
\begin{itemize}
  ...
  \item scaling is instantaneous;\caution{``Instantaneous'' is the ideal,
    not the mechanism: a virtual machine needs tens of seconds to boot and a
    container about a second, and an autoscaler that watches one-minute
    metrics waits for several of them, so capacity follows demand with a lag
    of minutes.}
...
which it runs, and its data reside on machines that others operate.\caution{%
Renting capacity does not remove the provisioning decision; it only shortens
its horizon.  How many instances to run must still be decided, now every few
minutes rather than every few years.}
...
\begin{description}
  ...
  \item[Billing and accounting] Usage is metered and billed.  Many pricing
    ...
    amounts of memory.\margin{Real prices, for scale: EC2's first instance
    type, \texttt{m1.small}---one virtual core and \SI{1.7}{\giga\byte} of
    memory---cost \$0.10 per hour on demand, and spot capacity is discounted
    by up to \SI{90}{\percent}.}
...
grows.\tip{The relative variation falls only as $1/\sqrt{N}$: ten times as
many independent customers reduce it by a factor of about~3, a hundred times
by a factor of~10.}  A provider can therefore provision for the aggregate
demand, which is far smaller than the sum of the individual peaks.
...
\begin{example}
  ...
  of the time and occur independently of the peaks of other
  customers.\caution{Independence is the assumption that does the work.
  Real demands share a daily cycle, holidays and live events, so peaks do
  arrive together; that is what reserve capacity is for, and why spot
  instances are reclaimed at two minutes' notice.}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l16-service-models}
...
\begin{enumerate}
  ...
  \item \textbf{Dealing with failures.}  At this scale failures are the
    normal case, and the system must continue to operate despite
    them.\sidenote{Google's study of more than \num{100000} of its own disks
    measured an annualized failure rate of \SI{1.7}{\percent} for drives in
    their first year and above \SI{8}{\percent} for three-year-old ones
    \cite{pinheiro2007}.  At \SI{5}{\percent} a year, a fleet of
    \num{100000} disks loses about 14 of them every day---before any
    machine, switch or power supply is counted.}
...
Failures become frequent simply because of the number of
components.\tip{For small $p$, $(1-p)^{N} \approx e^{-Np}$, so
$P_{\mathrm{fail}} \approx 1 - e^{-Np}$: about $Np$ while $Np \ll 1$, and
above \SI{95}{\percent} once $Np \ge 3$.}  If
...
components, $Np$, grows linearly with~$N$.\caution{\Cref{eq:l16-failure}
assumes that the components fail independently.  Correlated failures---a loss
of power or cooling, a network partition, a faulty software update---take down
many machines at once and are the ones that cause outages.}  A component that
rarely fails on its own therefore fails somewhere in a datacenter all the
time.  Systems
...
\margin{Hadoop is the open-source counterpart of the stack Google described:
HDFS corresponds to the Google File System, Hadoop MapReduce to MapReduce,
HBase to Bigtable.}
...
than overwritten;\margin{Such systems work in large pieces: GFS used chunks
of \SI{64}{\mega\byte} and HDFS uses blocks of \SI{128}{\mega\byte}, each
stored on three machines by default, so that a scan is sequential and one
lost machine costs no data.} in NoSQL databases, which scale out by giving
up the relational data model and some of the consistency guarantees of traditional
...
\begin{definition}[Software-defined networking]
  ...
  interface such as OpenFlow \cite{mckeown2008}.\margin{OpenFlow,
  specified in 2008 and the first widely implemented such interface, lets
  the controller install match-and-act rules in the forwarding tables of a
  switch.}
\end{definition}
...
of other tenants.  Software-defined storage and
software-defined datacenters apply the same principle to storage and to all
...
\begin{description}
  ...
  \item[Caching] keeps frequently used data in memory, avoiding the latency of
    the storage layer.\margin{The layers are orders of magnitude apart: a
    memory reference costs about \SI{100}{\nano\second}, a random read from
    flash about \SI{100}{\micro\second}, and a disk seek about
    \SI{10}{\milli\second}.}
...
it can use HDFS and other storage systems below it.\margin{Some components
have since been renamed or replaced: Tachyon became Alluxio in 2016, and
Shark was superseded by Spark SQL.  \Cref{tab:l16-stacks} keeps the original
names.}
