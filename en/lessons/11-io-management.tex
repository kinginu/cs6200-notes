...
enable or disable the device, or grant additional processes access to
it.\sidenote{Kernel bypass is routine in high-performance networking:
InfiniBand and RoCE adapters expose queue pairs to a process through the
verbs API, and DPDK drives Ethernet adapters entirely from user space, the
kernel only setting them up.  Lesson~15 uses such adapters for remote direct
memory access.}
...
\begin{itemize}
  ...
  \item If several processes use the device, the device must be able to
    ...
    receiving process.  With bypass the device itself must do
    so.\margin{PCIe devices do this with SR-IOV: one card presents many
    virtual functions, each with its own registers and queues, mappable into
    a different process or virtual machine (Lesson~12).}
\end{itemize}
