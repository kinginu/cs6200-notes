...
\caution{A device register is not a CPU register: it belongs to the device
and is named by an address, not by a register name in an instruction.}
...
switches.  A PCIe~4.0 lane carries about \SI{2}{\giga\byte\per\second} each
way, a $\times 16$ slot about \SI{32}{\giga\byte\per\second} (PCI-SIG).}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l11-interconnect}
...
\begin{definition}[Memory-mapped I/O]
  ...
  memory.\caution{A memory-mapped register is not ordinary memory: the
  region is mapped uncached, and drivers use accessors such as
  \texttt{readl} and \texttt{writel}, not plain pointer dereferences.}
\end{definition}
...
register, or receives the value read from it.\margin{The x86 port space is
only \SI{64}{\kibi\byte} wide---port numbers are 16 bits---which is one
reason why memory-mapped I/O carries most of today's device traffic.}
...
handled.\sidenote{Linux network drivers switch between the two with NAPI: on
the first packet the driver disables the interface's receive interrupts and
the kernel polls the device until its backlog is drained, then re-enables
them.  A lightly loaded interface is thus interrupt-driven and a busy one
polled.}
...
the device could read or overwrite whatever page had replaced the
buffer.\margin{A raw physical address also gives the device the run of
memory; platforms therefore interpose an IOMMU (Intel VT-d, AMD-Vi), and the
driver maps the buffer for DMA instead of handing over the address.}
...
\emph{user-level driver}, a library linked into the
application.\sidenote{Kernel bypass is routine in high-performance
networking: InfiniBand and RoCE adapters expose queue pairs to a process
through the verbs API, and DPDK drives Ethernet adapters entirely from user
space, the kernel only setting them up.  Lesson~15 uses such adapters for
remote direct memory access.}  The operating system retains coarse-grained
control: it can, for example, enable or disable the device, or grant
additional processes access to it.
...
\begin{itemize}
  ...
  \item If several processes use the device, the device must be able to
    \emph{demultiplex}, that is, determine to which process data
    belong.\margin{PCIe devices do this with SR-IOV: one card presents many
    virtual functions, each with its own registers and queues, mappable into
    a different process or virtual machine (Lesson~12).}  In a typical
    device access the kernel does this: its network stack, for example,
    inspects the headers of an arriving packet to find the receiving
    process.  With bypass the device itself must do so.
\end{itemize}

\section{Synchronous and asynchronous access}
\label{sec:l11-sync}

While a device carries out a request,\source{P3L5, sync vs.\ async access;
Silberschatz et al.\ ch.~12.} the thread that issued it can either wait or
continue.
...
does.\margin{glibc implements POSIX \texttt{aio\_read} with exactly this
helper-thread emulation; \texttt{io\_uring} (Lesson~6) is the kernel
interface that makes it unnecessary.}

\begin{figure}[htbp]
  \centering
  \input{figures/l11-sync-async}
...
\begin{definition}[File descriptor]
  ...
  locking and \texttt{close}.\margin{Descriptors 0, 1 and 2 are the standard
  input, output and error of a process; a process may hold \num{1024} open
  files by default (\texttt{ulimit -n}).}
\end{definition}
...
\begin{definition}[Inode]
  ...
  \caution{Several names can share one inode (a hard link); removing a name
  only decrements the link count, and the file is freed when the count
  reaches zero and no descriptor is open.}
\end{definition}
...
them.\margin{A group is not a region of the disk geometry: it holds as many
blocks as one block of bitmap can track---with \SI{4}{\kibi\byte} blocks,
$8 \times 4096 = \num{32768}$ blocks, that is \SI{128}{\mebi\byte}.}

\begin{figure}[htbp]
  \centering
  \input{figures/l11-ext2}
...
A file is identified by its inode,\source{P3L5, inodes, inodes with
indirect pointers; OSTEP ch.~40; Kerrisk ch.~14.} and the inode is the index
of all disk blocks of the file: it lists the blocks, in order, together with
the metadata.  In its simplest form the inode contains the block numbers
...
each indirect kind.\margin{Real ext2 numbers: a \SI{128}{\byte} inode, one
per \SI{16}{\kibi\byte} of partition, and 4-byte block numbers, so
$P = 1024$ with \SI{4}{\kibi\byte} blocks (\texttt{mke2fs.conf(5)}).}

\begin{figure}[htbp]
  \centering
  \input{figures/l11-inode}
...
four.\tip{To see which pointer reaches block $i$ of a file, compare $i$ with
the running sums $n_d$, $n_d + P$ and $n_d + P + P^2$: the first one it
falls below names the pointer class.}
...
(\cref{tab:l11-optim}).\margin{The gap these techniques attack: a
\num{7200}~rpm disk reads over \SI{100}{\mega\byte\per\second} sequentially,
but random \SI{4}{\kibi\byte} accesses, at roughly 100 per second, amount to
a few hundred kilobytes per second.}
...
\begin{definition}[Buffer cache]
  ...
  process requests it with \texttt{fsync}.\margin{The cache has no fixed
  size: on Linux it is the page cache, which keeps whatever memory the
  replacement policy of Lesson~8 leaves to it.}
\end{definition}
...
\begin{example}
  ...
  pairs 12, 13 and 90, 91 become sequential writes.\margin{Ordering by block
  number is the elevator (SCAN) algorithm; Linux ships
  \texttt{mq-deadline}, BFQ and Kyber, and usually runs NVMe with
  \texttt{none}.}
\end{example}
...
head movement.\margin{Linux reads \SI{128}{\kibi\byte} ahead by default; the
window is per block device, in \texttt{/sys/block/*/queue/read\_ahead\_kb}.}
...
applied afterwards.\caution{Journaling saves no writes---every logged block
is written twice---but replaces scattered writes with one sequential write
plus a later, batched update.}  The ext3 and ext4 file systems add such a
journal to the design of ext2; by default they log only metadata blocks and
write file data directly to their final locations, and logging the data as
well is an option.

\begin{figure}[htbp]
  \centering
  \input{figures/l11-journal}
