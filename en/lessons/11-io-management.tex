...
configuration space determine how large its region of physical memory is
and where the region lies; they are configured during the boot process.
Memory-mapped I/O must not be confused with a memory-mapped file
...
DMA.\tip{DMA pays off for large transfers, as for disks and network
interfaces; PIO remains adequate for devices that transfer a few bytes at a
time.}  A further requirement arises because the device accesses the buffer
by its physical address: the buffer must remain in physical memory until
...
block device stack; OSTEP ch.~36, 39; Bovet and Cesati \cite{bovet2005}
ch.~14.}  They use
files, the logical storage units, through the POSIX API with operations
such as \texttt{open}, \texttt{read}, \texttt{write} and \texttt{close}.
...
The same file system therefore works on every kind of block device, and a
block device driver implements a single interface towards the kernel while
it speaks the protocol of its device, such as SCSI or ATA, towards the
hardware.
...
systems.\source{P3L5, virtual file system; Kerrisk ch.~14; Bovet and Cesati
ch.~12.}  The files may reside on more than one device; different devices
...
OSTEP ch.~39--40; Bovet and Cesati ch.~12.}  The \emph{file} is the element
...
The data and properties of the file itself are recorded in its inode.
...
correspond closely to the abstractions of the VFS.\source{P3L5, ext2; Card
et al.\ \cite{card1994}; OSTEP ch.~41; Bovet and Cesati ch.~18.}  The second
extended file system, \term{ext2}[ext2], was for many years the default file
system of Linux and is the basis of its successors ext3 and ext4.  An ext2 partition begins with a block reserved
for booting, block~0; the rest is divided into groups of blocks
(\cref{fig:l11-ext2}).
...
\begin{description}
  \item[a superblock] recording the number of inodes, the number of disk
    blocks and the start of the free blocks;
  \item[a group descriptor] recording the locations of the bitmaps, the
    number of free inodes and the number of directories in the group;
  \item[a block bitmap and an inode bitmap] tracking which blocks and which
    inodes of the group are free;
  \item[the inodes] numbered from 1 to the maximum number, one per file;
  \item[the data blocks] holding the file data.
\end{description}
Keeping the inodes and the data blocks of a file in the same group places
related blocks close together on the disk, as the cylinder groups of the
Berkeley Fast File System do, and reduces the distance the disk head must
travel between them.

\begin{figure}[htbp]
  \centering
  \input{figures/l11-ext2}
  \caption{Layout of an ext2 partition.  After the boot block, the partition
    is divided into block groups, each with its own superblock, group
    descriptor, bitmaps, inodes and data blocks.}
  \label{fig:l11-ext2}
\end{figure}
...
\end{lstlisting}
\caution{A successful \texttt{write} does not mean the data are on disk.
Data still waiting in the buffer cache are lost if the system crashes.}
...
problems.  The
file system describes each update---which block, at which offset, with which
value---in a log, the \emph{journal}, which it writes sequentially to a
...
applied afterwards.  The ext3 and ext4 file systems add journaling to the
design of ext2.
