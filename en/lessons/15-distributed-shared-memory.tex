...
\lessoninfo{
  video={P4L3},
  textbook={Nitzberg and Lo \cite{nitzberg1991}; OSTEP \cite{ostep} ch.~48; Tanenbaum and Bos \cite{tanenbaum2015} ch.~8; Tanenbaum and van Steen \cite{tanenbaum2007} ch.~7},
...
clearly divided.\source{P4L3, lesson preview, visual metaphor, reviewing
distributed file systems, peer distributed applications.}  Servers own and
manage the state, the files, and provide the service, file access; clients
...
analytics, web search or content sharing, the state of the application is
...
Managing state that is spread over peers raises three questions, which
recur throughout this lesson:
\begin{description}
  \item[Placement] On which node should each part of the state reside?
    Access is fastest when the state resides on the node of the processes
    that access it most.
  \item[Migration] When a process accesses state that resides on another
    node, how and when is the state brought to the process's node, by
    moving it or by copying it?
  \item[Sharing rules] When several nodes access the same state, which
    rules determine the order in which their operations take effect, and
    when each node observes the updates made by the others?
\end{description}
Distributed shared memory, a peer distributed application whose state is
memory, is the example on which the lesson studies these questions.
...
\cite{nitzberg1991}; OSTEP ch.~48; Tanenbaum and Bos ch.~8.}  Distributed
...
moderate memories.  The price is latency.  An access to memory that
resides on another node crosses the network, which is orders of magnitude
slower than an access to local memory.  Commodity interconnects reduce this
cost by supporting remote direct memory access.
...
neither machine lies on the data path.\margin{InfiniBand, RoCE and iWARP
are interconnects that provide RDMA.}

\section{Hardware and software DSM}
\label{sec:l15-hwsw}

DSM can be provided by hardware, by software, or by a combination of
both.\source{P4L3, hardware vs.\ software DSM, DSM quiz; Nitzberg and Lo
\cite{nitzberg1991}.}
\begin{description}
  \item[Hardware-supported DSM] relies on a special-purpose interconnect.
    ...
    node, and the NICs take part in all aspects of managing the shared
    memory.  Such hardware is expensive; it is used in high-end systems
    where performance is paramount, as in high-performance computing.
  \item[Software-supported DSM] performs everything in software: locating
    ...
    system or of the runtime of a programming language; the two share
    different units of memory (\cref{sec:l15-granularity}).
...
\begin{description}
  ...
  \item[Pages] A page, commonly \SI{4}{\kibi\byte}, is the unit that the
    operating system already manages.  A page-based DSM is implemented by
    the operating system in cooperation with the MMU
...
\begin{figure}[htbp]
  \centering
  \input{figures/l15-false-sharing}
...
\begin{description}
  ...
  \item[MRMW] Under
    ...
    same order and reads return the most recent write.
\end{description}
...
vs.\ replication, migration vs.\ replication quiz; Nitzberg and Lo.}  Since
a remote access is orders of magnitude slower than a local one, even a
small fraction of remote accesses dominates the average.
...
should reside on the process's own node.  Two techniques bring memory to
...
\begin{description}
  \item[Migration] The DSM keeps a single copy of each unit and moves it to
    ...
    the same time.
  \item[Replication] The DSM copies a unit to several nodes, each of which
...
\begin{figure}[htbp]
  ...
  \caption{Bringing page $p$ to the nodes that access it.  (a) Migration
    ...
    a write by the owner A invalidates the replicas on B and C.}
...
\begin{example}
  ...
  \SI{0.05}{\milli\second}.  Migration moves $p$ on every access by a node
  that does not hold it: 7 transfers, \SI{3.5}{\milli\second}.  With
...
when reads dominate.  When replicated units are written often, the
consistency management that every write triggers can cost more than the
locality of the replicas saves.
...
\begin{description}
  \item[Push] When a replica is written, the DSM immediately pushes
    ...
    read and cause harm, and removes stale replicas as soon as they arise.
...
for the shared state (\cref{sec:l15-models}).

\begin{figure}[htbp]
  \centering
  \input{figures/l15-push-pull}
...
\begin{description}
  \item[Home node] The node that manages the page, also called its
    ...
    whether it is locked, and where its replicas reside.
  ...
  \item[Explicit replicas] Copies that the DSM creates deliberately, rather
    ...
    kept in three copies.  The home node controls where these replicas are
    placed and how they are managed.
\end{description}
...
\begin{itemize}
  \item Every page is identified by an address composed of a node ID and a
    page frame number\index{page frame number} (PFN); the node ID names the
    page's home node, and the PFN the frame on that node.
  ...
  \item The metadata of the pages themselves are partitioned: each manager
    keeps the metadata of the pages it manages.
\end{itemize}
...
the page IDs, and every reference that uses them, remain unchanged.

\begin{figure}[htbp]
  \centering
  \input{figures/l15-indexing}
...
\begin{example}
  ...
  Page \texttt{0x050012A3} is frame \texttt{0x0012A3} of home node~5.  The
  mapping table is indexed by the node ID field and initially maps each
  index $i$ to node~$i$.  When node~5 becomes overloaded, the management of
  its pages is handed to node~9 by setting entry~5 to 9 on all nodes and
  moving the page metadata from node~5 to node~9
  (\cref{fig:l15-indexing}).  No page ID changes.  Had the manager been
  taken directly from the node ID field, every one of these pages would
  have needed a new ID, and every reference to it would have had to be
  updated.
\end{example}
...
reasons.\source{P4L3, implementing DSM; Nitzberg and Lo; OSTEP ch.~48.}
When a process accesses memory that is not present on its node, the DSM
...
every other access completes in hardware.  The DSM sets up the mappings so
that exactly the accesses it must intercept fault.
\begin{itemize}
  \item A page that is not present on the node has no valid mapping.  An
    ...
    DSM, which requests the page from its owner, installs a mapping for the
    received copy and resumes the process.
  \item A page of which the node holds a replica is mapped read-only
    ...
    over ownership---before it enables writing and resumes the process.
\end{itemize}
...
\begin{example}
  ...
  access with invalidation on write.  When node~2 obtains a replica in
...
with time increasing to the right; all locations initially hold 0.
...
must see the same interleaving.
...
these dependencies.}  In exchange, it needs to coordinate only the order of
causally related updates.
...
needs.  Weak consistency limits data movement and coordination to the
synchronization points, at which the updates made since the previous point
can be propagated together.  In exchange, the DSM must maintain extra state
...
applications fewer guarantees.

\begin{figure}[htbp]
  \centering
  \input{figures/l15-models}
  \caption{Operations of \cref{ex:l15-models}.  P2's read of $m_1$ (grey)
...
\begin{example}
  \label{ex:l15-models}
  In \cref{fig:l15-models}, P1 writes 1 to $m_1$ at $t_1$ and P2 writes 2 to
  $m_2$ at $t_2$.  P3 and P4 read both locations at $t_3$ and $t_4$, in
  ...
  \begin{itemize}
    ...
    \item Outcome (b) is not: P4 reads 0 from $m_2$ at $t_3$, after 2 was
      written at $t_2$.  It is sequentially consistent, because the order
      $R_{m_1}(0)$ by P3, $R_{m_2}(0)$ by P4, $W_{m_1}(1)$, $W_{m_2}(2)$,
      $R_{m_2}(2)$ by P3, $R_{m_1}(1)$ by P4 explains every read and
      preserves the order of each process's operations.
    \item Outcome (c) is not sequentially consistent: P3 observes
      $W_{m_1}(1)$ before $W_{m_2}(2)$, P4 observes them the other way
      round, and no single order explains both.  If the two writes are
      concurrent, (c) is causally consistent.  If P2 read 1 from $m_1$
      before writing $m_2$, however, $W_{m_2}(2)$ causally depends on
      $W_{m_1}(1)$, and P4, which reads 2 from $m_2$ and afterwards 0 from
      $m_1$, observes the effect before its cause.
  \end{itemize}
  With a synchronization point at $t_5$, all three outcomes are weakly
  consistent, because no guarantee applies before $t_5$; the read of $m_1$
  by P4 at $t_6$, however, must return 1.
\end{example}
...
\begin{table}[htbp]
  ...
  \caption{Outcomes of the reads at $t_3$ and $t_4$ in \cref{ex:l15-models}.
    ...
    reading $m_1$ before writing $m_2$.}
  ...
  \begin{tabular}{@{}lllcccc@{}}
    ...
    \midrule
    (a) & $R_{m_1}(1)$, $R_{m_2}(2)$ & $R_{m_2}(2)$, $R_{m_1}(1)$ & yes & yes
      & yes / yes & yes \\
    (b) & $R_{m_1}(0)$, $R_{m_2}(2)$ & $R_{m_2}(0)$, $R_{m_1}(1)$ & no & yes
      & yes / yes & yes \\
    (c) & $R_{m_1}(1)$, $R_{m_2}(0)$ & $R_{m_2}(2)$, $R_{m_1}(0)$ & no & no
      & yes / no & yes \\
    \bottomrule
  \end{tabular}
\end{table}
