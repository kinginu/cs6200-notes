...
\begin{description}
  ...
  \item[Replication] The DSM copies a unit to several nodes, each of which
    ...
    of each unit.\tip{Count messages: with $n$ replicas a write costs $n-1$
    invalidations, and the next access on each invalidated node costs a
    fetch.  Replication pays while the local reads it saves outnumber
    them.}
\end{description}
