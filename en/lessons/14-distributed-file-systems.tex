...
\lessoninfo{
  video={P4L2},
  textbook={OSTEP \cite{ostep} ch.~49--50; Silberschatz et al.\ \cite{silberschatz2018} ch.~15, 19; Nelson et al.\ \cite{nelson1988}},
...
systems, DFS models; Silberschatz et al.\ ch.~19.}  Beneath
...
ch.~49--50; Silberschatz et al.\ ch.~19.}  Two extremes bound the design space
(\cref{fig:l14-remote}).
...
\begin{example}
  ...
  costs a round trip of \SI{1}{\milli\second}.\caution{Network rates are
  quoted in bits per second: \SI{1}{\giga\bit\per\second} carries at most
  \SI{125}{\mega\byte\per\second}, so the rate assumed here is a
  well-filled gigabit link.}  If the client reads 50
  ...
  \SI{10.2}{\second} and every read after it is local.\tip{Downloading wins
  once the client reads more than $S/(B\,t_{\mathrm{RTT}})$ blocks---the
  number of round trips that fit into one download, about \num{10000} here.}
\end{example}
...
as long as the requests are idempotent (Lesson~13), as a read or a write at
an explicit offset is.  Its disadvantages are that it cannot support
caching with consistency management, nor locking, and that every request
must describe the operation completely---which file, which offset, how many
bytes---so that more bits are transferred per request.\caution{Stateless
refers to state about \emph{clients}.  The server still holds the files and
their metadata; what it forgets between requests is who is reading what.}
Its clients may
still cache data and ask the server, when they open a file or
periodically, whether their copy is current (client-driven validation,
\cref{sec:l14-caching}, as in NFS), but the server can neither notify them
of a change nor guarantee that their copies are coherent.
...
\begin{example}
  ...
  offset with every read; with a file handle of 32 bytes (the fixed size
  in NFSv2), an 8-byte offset and a 4-byte count, each request carries 44
  bytes of arguments.  The stateful server records the open
...
\begin{keypoint}
  ...
  simpler and survives failures by restarting, but cannot guarantee that
  client caches are consistent.
\end{keypoint}
...
DFS; OSTEP ch.~49--50; Silberschatz et al.\ ch.~19.}  A client holds these
blocks in its own memory, like the buffer cache of Lesson~11: the fastest
place, but one that is limited in size and lost when the client
crashes.\margin{The clients of NFS (\cref{sec:l14-nfs}) and of Sprite
(\cref{sec:l14-sprite}) cache blocks in memory.}  On a local disk or SSD a
client can cache more data and keep them across a crash, at the price of
being slower than memory, though still faster than the network.  The
server caches blocks as well, in its own buffer cache, which saves it disk
accesses; how much it saves depends on the load, since the requests of
many clients for different files interleave there and displace one
another's data.\caution{`Faster than the network' assumes a slow network: a
random block of a \num{7200}~rpm disk costs about \SI{13}{\milli\second}
(OSTEP ch.~37), more than a round trip inside a datacenter.}  Once the same
file is cached at several places, the copies can diverge.  Suppose that clients~1 and~2 both cache file~$F$ and
that client~2 modifies its copy into $F'$ (\cref{fig:l14-caching}).  The
modification must reach the server, and client~1 must learn that its copy
of $F$ is stale.

\begin{figure}[htbp]
  \centering
  \input{figures/l14-caching}
...
copies of a memory location in the caches of the other CPUs stale and, on
a cache-coherent platform, the hardware updates or invalidates them.  In a
DFS the coherence mechanism is implemented in software, and it is either
\begin{description}
  ...
  \item[server-driven] the server, which knows which clients cache a file,
    notifies them when the file changes.\sidenote{The Andrew File System,
    built at Carnegie Mellon at the same time as Sprite, carries the
    server-driven design further: a client caches whole files on its local
    disk, and the server holds a \emph{callback} promise to warn it before
    the file changes, so that an open costs nothing while the promise
    stands.  OSTEP ch.~50 describes it.}
\end{description}
...
al.\ ch.~15; OSTEP ch.~49--50.}  In a DFS whose clients cache data, a write by
...
new file.\margin{The same device as the copy-on-write of Lesson~8: the new
version is written elsewhere and the old blocks are freed later.}  Finally,
a DFS can offer \emph{transactions}: an application groups several updates, of one or several files, into a transaction, and
...
vs.\ directory service.}  The right choice
...
The NFS client keeps the handle with its representation of the open
file\margin{The handle is \SI{32}{\byte} and of fixed size in NFSv2; later
versions make it variable, at most \SI{64}{\byte} in NFSv3 \cite{rfc1813}
and \SI{128}{\byte} in NFSv4 \cite{rfc7530}.}
...
manager.\sidenote{NFSv3 is really a family of RPC programs: NFS itself
\cite{rfc1813}, a MOUNT protocol that turns a path into the handle of an
exported directory, a lock manager, and a status monitor with which the two
ends tell each other that they have rebooted.  Each is registered with
\texttt{rpcbind} (Lesson~13); NFSv4 folds them all into one protocol on
port 2049.}  NFSv4 is stateful: opening files, locking and the delegations
...
mount options \texttt{acregmin} and \texttt{acdirmin}; \texttt{acregmax} and
\texttt{acdirmax}, both \SI{60}{\second} by default, bound the interval from
above.}  The longer period
for directories is deliberate: the contents of a directory change far less
often than the data of a file, while lookups in it are frequent, so
checking it rarely saves many requests and costs only that a name created
or removed by another client is seen late.  The policy is the opposite of
the example in \cref{sec:l14-dirs}, and it is acceptable because the
operations that modify a directory go to the server in any case and are
serialized there.
...
\begin{note}
  NFS thus combines session semantics with periodic revalidation.\margin{The
  session-like half is called \emph{close-to-open} consistency: revalidate at
  open, write back at close.  The \texttt{nocto} mount option turns off the
  check at open.  \texttt{nfs(5)}.}  A
  periodic check can bring the changes of another client into a session
  that is still open, which pure session semantics excludes, and between
  two checks a client may read data that has already changed at the
  server; UNIX semantics is therefore not provided.
\end{note}

\subsection{Locking}

Locking in NFSv4 is based on leases.\margin{The lock manager of NFSv3 is a
protocol of its own, the Network Lock Manager (NLM).}  The server grants a
lock for a limited period, and before this lease expires the client must
either release the lock or renew the lease.
...
(Lesson~10); a client can upgrade its reservation from reading to writing,
or downgrade it again, without closing the file.

\begin{figure}[htbp]
  \centering
  \input{figures/l14-lease}
...
\begin{example}
  A server grants leases of \SI{30}{\second}.\margin{The protocol fixes no
  duration: all leases of one server have the same length, which a client
  reads from the server's \texttt{lease\_time} attribute \cite{rfc7530}.}
  Client~1 acquires a lock at
  ...
  granted to another client.\caution{Leases need no synchronized clocks: the
  client starts its timer only when the grant arrives, so it always gives up
...
The distributed operating system \term{Sprite}[Sprite] was developed at
the University of California, Berkeley, in the 1980s; its file system let
...
\begin{itemize}
  \item Clients cache file blocks and write modified blocks back to the
    server with a delay of 30 seconds, as described below.\margin{Linux does
    the same for its page cache: flusher threads wake every
    \SI{5}{\second} and write back data that has been dirty for more than
    \SI{30}{\second} (\texttt{dirty\_expire\_centisecs}).}
...
after its last modification.\caution{Delayed write trades durability for
traffic: a client crash loses every block not yet written back, up to a
minute of work.  Sprite accepts this because most of those blocks would
...
that opens it sees the data written by the previous writer.\caution{%
\emph{Sequential} here describes the clients, not the access pattern within
the file: one client after another, not blocks read in order.}  Under
...
\begin{example}
  ...
  sequential, and $W_2$ may cache $F$.\tip{Caching survives sequential
  sharing because there is always a last writer whose blocks the server can
  collect first; under concurrent sharing there is none, so the server must
  serve every access itself.}  In step~6 $W_3$ opens $F$ while
