...
bytes---so that more bits are transferred per request.  Its clients may
...
another's data.  Once the same file is cached at several places, the
copies can diverge.  Suppose that clients~1 and~2 both cache file~$F$ and
...
mount options \texttt{acregmin} and \texttt{acdirmin}.}  The longer period
...
\begin{note}
  NFS thus combines session semantics with periodic revalidation.  A
