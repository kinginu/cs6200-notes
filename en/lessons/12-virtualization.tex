...
\begin{note}
  ...
  platform, not the real machine, and it executes every instruction in
  software, by interpretation or translation, none of them directly on
  the processor, so it does not meet the performance requirement.
\end{note}
...
remained confined to them until the late 1990s
\cite{rosenblum2005}.\sidenote{The first such system was CP-40 with CMS, run
by IBM's Cambridge Scientific Center from January 1967; CP-67 followed on the
System/360 Model~67, whose hardware translated addresses.  The line continued
as VM/370, announced in 1972, and survives as z/VM.}
Mainframes were expensive and few.  Once commodity hardware had become
cheap and its operating systems supported multitasking, it was simpler to
buy another
...
created the problems that revived them: datacenters filled with machines
that each ran a single application and were underutilized, so that the
space, power and administration they consumed grew out of proportion to
the work they did.  Consolidating the applications onto fewer machines
addresses these costs at once, but it requires virtualizing commodity
...
\begin{example}
  ...
  averaging \SI{8}{\percent} CPU utilization.\margin{Low utilization is
  the norm: Barroso and H\"olzle report that datacenter servers spend most of
  their time between \SI{10}{\percent} and \SI{50}{\percent} CPU
  utilization (\emph{The Datacenter as a Computer}).}  Their combined load
...
tasks, so that the hypervisor itself stays small.\caution{The type~1 and
type~2 labels do not split cleanly.  KVM is a module of Linux, so its
hypervisor does run on the hardware; it counts as type~2 because a complete
host OS remains around it.}

\begin{figure}[htbp]
  \centering
  \input{figures/l12-models}
...
The hypervisor \term{Xen}[Xen] \cite{barham2003}, available as open source
and as the commercial Citrix XenServer, follows this design.  Its VMs are
called \emph{domains}.  The privileged service VM is \term{dom0}[dom0],
which runs Linux with the device drivers and the management tools; the
guest VMs are unprivileged domains, \emph{domU}, and the Xen hypervisor
itself contains no drivers for I/O devices such as disks and network
cards.\margin{In the first Xen version, the one described by Barham et
al.\ \cite{barham2003}, these drivers still resided in the hypervisor.}
VMware ESX takes the other route and places the drivers
...
of privileged operations that the guest performs.\margin{The hardware
transition alone is below 500 cycles on Haswell; a \texttt{VMCALL} into KVM
and back measures about \num{1500} cycles (kvm-unit-tests
\texttt{vmexit}).}

\begin{figure}[htbp]
  \centering
  \input{figures/l12-trap-emulate}
...
\cite{robin2000}.}
The architecture did not satisfy the condition of trap-and-emulate: Robin
and Irvine \cite{robin2000} identified 17 sensitive instructions that do not
trap when executed in a less privileged ring.\caution{The trick fails for
64-bit guests: AMD64 long mode dropped segment limit checks, so a software
VMM can no longer fence the guest off by segmentation.  They need the
support of \cref{sec:l12-hw}.}  Executed in ring~1, they fail
silently or reveal machine state that the hypervisor intends to hide, and
trap-and-emulate alone cannot virtualize the processor correctly.
...
value, without a trap.\margin{The hypervisor sets the I/O privilege level
to~0, so \texttt{POPF} in ring~1 leaves IF unchanged, while \texttt{CLI}
traps there.}  The hypervisor is not invoked, so it does
...
\begin{definition}[Binary translation]
  ...
  guest code at run time, just before it executes.  Problematic
  instructions are translated into an alternative instruction sequence
  with the intended effect; all other instructions are kept as they are
...
\begin{enumerate}
  ...
  \item Otherwise take the block over unchanged, with only its control
    transfers adjusted, so that it runs at hardware speed while execution
    stays under the control of the translator.
\end{enumerate}
The resulting blocks are kept in a \emph{translation cache}, so the cost of
...
\begin{figure}[htbp]
  \centering
  \input{figures/l12-binary-translation}
...
\begin{example}
  A block of guest kernel code executes natively in
  \SI{5}{\micro\second}.\sidenote{Binary translation did not become obsolete
  when hardware support arrived: on the first VT-x processors the software
  VMM still won on most workloads, where exits were expensive and page
  tables had to be shadowed either way \cite{adams2006}.}
...
(\cref{fig:l12-memory}).\tip{Read the two arrows in order: virtual
$\to$ physical belongs to the guest, physical $\to$ machine to the
hypervisor.  The shadow page table is their composition, virtual $\to$
machine.}

\begin{figure}[htbp]
  \centering
  \input{figures/l12-memory}
...
\begin{itemize}
  \item It write-protects the memory that holds the guest page tables.
    ...
    the corresponding machine address.\margin{Write-protecting a page in
    order to learn about a write is the mechanism of copy-on-write in
    Lesson~8, used here to observe the guest rather than to copy a page.}
...
machine addresses \cite{barham2003}.\caution{The names differ between
systems: what Xen and this lesson call physical and machine addresses, Intel
calls guest-physical and host-physical, and VMware guest physical and host
physical.}  The guest registers its page tables
...
\begin{itemize}
  \item Sharing the device is difficult: it must be reassigned from one VM
    to another.\margin{SR-IOV (PCI-SIG, 2007) softens this: one card
    presents a physical function and a number of virtual functions---up to
    256 by the specification, in practice tens---each of which can be
    assigned to a VM.}
...
The key benefit is that the VM is decoupled from the physical
device.\margin{The emulated devices are deliberately old and common: QEMU's
PC machine presents an Intel e1000 network card and an IDE disk controller,
for which every guest OS already has a driver.}  The
...
to send, to the back-end driver, in Xen through descriptor rings in memory
shared between the two VMs.\margin{Linux's virtio (Russell, 2008; an OASIS
standard since 2016) is the common front-end/back-end interface; its
\emph{virtqueues} are the shared memory of Lesson~9.}  This eliminates the
emulation overhead of the hypervisor-direct model, and because all requests
for a device pass through the back-end driver, the service VM can manage how
the device is shared among the VMs.  The price is a modified guest: its
drivers must be replaced by front-end drivers written for the hypervisor, so
the model
applies to paravirtualized guests and to guests in which such drivers have
been installed.  \Cref{tab:l12-devices} summarizes the three models.
...
\begin{table}[htbp]
  ...
  \small
  \begin{tabular}{@{}>{\raggedright}p{18mm}>{\raggedright}p{20mm}%
      >{\raggedright}p{21.5mm}>{\raggedright}p{20.5mm}%
...
\begin{description}
  ...
  \item[Extended page tables] The MMU performs the two-step translation of
    ...
    virtual to machine address.\tip{A walk through $g$ guest levels under
    $n$ extended levels costs $(g+1)(n+1)-1$ references: 24 for $g=n=4$,
    against 4 natively (AMD-V nested paging white paper).}  The guest OS
    changes and switches its page tables without VM exits, and shadow page
    tables become unnecessary.  AMD's equivalent is called nested page tables.
...
\end{description}
New instructions enter non-root mode, read and write the VM control
structure and issue hypercalls.\margin{On Intel: \texttt{VMLAUNCH} and
\texttt{VMRESUME} for VM entries, \texttt{VMREAD} and \texttt{VMWRITE} for
the control structure, \texttt{VMCALL} for hypercalls.}  Hosted hypervisors
such as KVM rely on them.
...
tables and tagged TLB entries.\margin{VT-x shipped in November 2005 with
the Pentium~4 662 and 672, AMD-V in May 2006.  The page table and TLB
support came later: nested page tables and ASID tags with Barcelona (2007),
extended page tables and the 16-bit VPID tag with Nehalem (2008).}
\emph{VT-d} (directed I/O) adds to the chipset an I/O memory management unit
that translates the addresses used in DMA (Lesson~11) and remaps interrupts,
so that a device assigned to a VM in the passthrough model cannot access the
...
that present themselves as several virtual devices, each assignable to
a VM.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}>{\raggedright}p{24mm}>{\raggedright}p{16mm}%
      >{\raggedright}p{29mm}>{\raggedright}p{20mm}%
...
\begin{keypoint}[Lesson summary]
  ...
  virtualization adds machine addresses beneath physical addresses, handled
  by shadow page tables or by validated, batched page table
