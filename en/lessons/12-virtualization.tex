...
shared between the two VMs.  This eliminates the emulation overhead of the
...
shared among the VMs.  The price is a modified guest: its drivers must be
replaced by front-end drivers written for the hypervisor, so the model
...
in DMA (Lesson~11) and remaps interrupts, so that a device assigned to a VM
in the passthrough model cannot access the memory of other
VMs.  \emph{VT-c} (connectivity) brings virtualization
