...
\lessoninfo{
  video={P2L4},
  textbook={OSTEP \cite{ostep} ch.~6, 26--28; Silberschatz et al.\ \cite{silberschatz2018} ch.~4; Kerrisk \cite{kerrisk2010} ch.~20--22},
...
that belongs to the individual thread has to be saved and restored.  Some
...
scalability, overhead, performance and flexibility at once.
...
\begin{figure}[htbp]
  ...
  \caption{The SunOS~5.0 threading model.  Each kernel-level thread that
    ...
    right perform kernel work and have none.  The kernel-level scheduler
    places kernel-level threads on CPUs.}
  \label{fig:l05-sunos}
\end{figure}
...
\begin{definition}[Red zone]
  ...
  that is not mapped to memory, so that any access to it causes a fault.
\end{definition}
...
\begin{example}
  ...
  \SI{4}{\mebi\byte} of red zones use no physical memory.
\end{example}
...
calls and special signals let the two levels coordinate.  The library can
tell the kernel how many kernel-level threads its process wants---its
\emph{concurrency level}---and can request more; the kernel can notify the
library of events that affect its threads, such as kernel-level threads
blocking.
...
\begin{example}
  ...
  the kernel adjust the number of kernel-level threads.
\end{example}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}c>{\raggedright\arraybackslash}p{0.66\linewidth}>{\raggedright\arraybackslash}p{0.18\linewidth}@{}}
    ...
    4 & U3 also issues a blocking request.  If K3 blocked as well, no
        kernel-level thread would be left to run U4 and the whole process
        would be blocked.  Before blocking K3, the kernel therefore
        notifies the library; the library requests another kernel-level
        thread, the kernel creates K4, and U4 runs on it. & 3 blocked,
        1 running \\
    5 & The I/O completes and U1--U3 continue.  K4 later stays idle for a
        long time; the kernel informs the library that K4 is no longer
        available. & K1--K3 \\
    \bottomrule
  \end{tabular}
\end{table}
...
thread to a CPU; neither decision is visible at the other level.
...
avoids many of these problems, because the kernel sees every thread.
...
them.  Independently of reuse, kernels bound the total number of threads,
because every thread occupies kernel memory; Linux derives its limit from
...
\begin{keypoint}
  ...
  explicit notification across the levels.
\end{keypoint}
...
system raises \texttt{SIGSEGV} (signal~11 on Linux).  For each process it
...
\begin{example}
  ...
  that stops its work loop when a timer expires:
...
was interrupted: the thread's program counter is set to the handler, and
the handler runs on the thread's stack.\source{P2L4, why disable
...
hardware.  Signal masks are kept per execution context, that is, per
user-level thread running on a kernel-level thread.  If the mask disables a
signal, the kernel does not interrupt that thread to deliver it.  A
process changes its signal mask with \texttt{sigprocmask}
for a single-threaded process and \texttt{pthread\_sigmask} for a thread,
each taking one of the operations \texttt{SIG\_BLOCK},
\texttt{SIG\_UNBLOCK} or \texttt{SIG\_SETMASK}.
...
too expensive, so the decision is made dynamically: a handler that does not
lock a mutex executes on the stack of the interrupted thread, and only a
handler that can block is turned into a real thread.  The thread
structures used for interrupts are created and initialized in advance.

The same idea underlies a common way of dividing the work of interrupt
handling (\cref{fig:l05-top-bottom}).
...
\begin{definition}[Top half and bottom half]
  ...
  the deferred part; it can be of arbitrary complexity and, executed like a
  thread, it is scheduled and may block.
\end{definition}

\begin{figure}[htbp]
  \centering
  \input{figures/l05-top-bottom}
...
Handling interrupts as threads is not free.\margin{Linux uses the terms
top half and bottom half in this sense.  In BSD terminology, the top half
of the kernel is the part entered through system calls and the bottom half
the part that handles interrupts.}  In SunOS~5.0 it adds about 40 SPARC instructions to
...
more often than interrupts occur, so the balance is positive.
...
mask of the user-level thread currently running on it.
...
\begin{description}
  ...
  \item[Case 4] No user-level thread has the signal enabled, but the masks
    ...
    again, and the pending signal is delivered.
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l05-signal-cases}
...
struct task_struct {       // abridged, Linux 3.x
    ...
    pid_t tgid;            // task group identifier
    int prio;
    volatile long state;
...
form a task group whose identifier, \texttt{tgid}, is the \texttt{pid} of
the first task of the group; this is the process identifier that
\texttt{getpid} returns.  The task structure does not contain the
...
(\cref{tab:l05-clone}).  With all these flags set, the new task is a thread
of the creating process; with all of them cleared, it is a new process, and
\texttt{fork} is implemented in this way.\margin{A thread library also
passes \texttt{CLONE\_THREAD}, which places the new task in the task group
of its creator.}
...
thread but did not conform to POSIX in areas such as signal handling; a
many-to-many implementation developed at the same time was abandoned in
its favour.

\begin{keypoint}[Lesson summary]
  The kernel and the user-level library split thread state into several
  structures: hard process state shared by all threads, light process state
  per kernel-level thread, and user-level thread structures, allocated
  contiguously with red zones.  SunOS~5.0 links them through LWPs, lets the
  library and the kernel coordinate through system calls and signals, uses
  adaptive mutexes on multiprocessors and reuses exited threads through a
  reaper.  Interrupts and signals run handlers in the interrupted thread's
  context; masks prevent a handler from deadlocking with its thread, and
  interrupts as threads, top and bottom halves, and the library's handling
  of user-level signal masks all optimize the common case.  Linux represents
  every thread as a task created with \texttt{clone}, and NPTL maps
  user-level threads one-to-one onto tasks.
\end{keypoint}
