...
\begin{itemize}
  ...
  \item dispatch simple, short tasks first: this maximizes the throughput,
...
\begin{itemize}
  ...
  \item a new task becomes ready, for example a newly created task or a task
    ...
    it;
  \item the timeslice of the running task expires.
\end{itemize}
...
The total work is the same in both orders, so the throughput of the batch
does not change.  By running short tasks first, however, SJF lets most
tasks complete early and lowers both averages.  In fact, for tasks that
...
average of the times it ran the last $n$ times.
...
(Lesson~5).  POSIX threads (Lesson~4) provide priority inheritance as a
protocol attribute of a mutex:
...
\begin{keypoint}
  ...
  of the tasks blocked on its lock.
\end{keypoint}
...
uninterrupted time given to a task; it is also called the time quantum.  A
...
\begin{example}
  ...
  time, round robin comes close to the average completion time of SJF, and
  since a task waits for at most one timeslice of each task ahead of it
  before it first runs, its average wait time is even shorter.
\end{example}
...
of a context switch, as \cref{eq:l02-useful} showed.
...
\begin{example}
  ...
  arrive at time 0; a context switch takes \SI{0.2}{\second}.  With a
...
are I/O-bound, the length of the timeslice hardly matters:
each task blocks for I/O before its timeslice expires, whatever the
timeslice.  The length matters when I/O-bound tasks share the CPU with
CPU-bound ones.

\begin{figure}[htbp]
  \centering
  \input{figures/l07-io-timeslice}
...
\begin{description}
  ...
  \item[I/O-bound tasks prefer shorter timeslices.]  An I/O-bound task runs
    ...
    performance, since interactive tasks are typically I/O-bound.
\end{description}

\subsection{CPU utilization}

The fraction of CPU time spent on useful work, \cref{eq:l02-useful},
extends to a mix of tasks with different behaviour.  Choose a recurring
...
\begin{example}
  ...
  so the I/O-bound tasks are always ready when their turn comes.
\end{example}
...
\begin{example}
  ...
  as 120; if it computes without sleeping, at a priority as low as 130.
\end{example}

\begin{figure}[htbp]
  \centering
  \input{figures/l07-o1}
...
It has been the default Linux scheduler for non-real-time tasks since
version 2.6.23.  Its runqueue is a \term{red-black tree}[赤黒木], a
...
\begin{example}
  ...
  factor of about 1.25; nice value 5 has weight 335.  A task of weight $w$
...
until that cache has been filled.
...
every memory access of the task.

\begin{example}
  A task spends \SI{40}{\percent} of its execution time accessing memory, and
...
on one CPU.  The CPU remains a single CPU, with a single pipeline, but it can
switch between these contexts very quickly.
...
most common configuration, and SMT can be enabled or disabled at boot time.
...
Which tasks should share a CPU?  Fedorova et al.\ answer the question with a
simple model of an SMT CPU based on four assumptions.\source{P3L1,
scheduling for hyperthreading platforms; Fedorova et al.}
\begin{enumerate}
  ...
  \item A memory access of a memory-bound thread takes 4 cycles, during which
    the thread issues no instructions.
...
Fedorova et al.\ asked whether a single counter-derived metric, the CPI,
suffices to distinguish the tasks.\source{P3L1, CPI experiment, CPI
...
while a CPU-bound task has a CPI close to 1.  A scheduler could then mix
...
every core ran threads of the same CPI.  Performance was measured in
instructions per second (IPS).

The assignments with mixed CPIs achieved the highest IPS, and those in
...
workloads, however, behave differently.  The CPIs of real applications, such
as those of standard benchmark suites, are not nearly as distinct: they
cluster in a narrow range.  CPI therefore cannot tell the tasks of a real
...
\begin{note}
  ...
  balance the load.  The use of the last-level cache over time turned out to
  be a better indicator of contention than CPI.
\end{note}
