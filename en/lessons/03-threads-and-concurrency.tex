...
\begin{figure}[htbp]
  \centering
  \input{figures/l03-process-thread}
...
\begin{example}
  ...
  \SI{1}{\mega\byte} of stack and other per-context state.  Run as four
...
therefore lets a program hide idle time even on one CPU.

\begin{figure}[htbp]
  \centering
  \input{figures/l03-idle-time}
...
\begin{example}
  ...
  figures are \SI{3.5}{\milli\second} and \SI{0.5}{\milli\second}.
\end{example}
...
The choice of notification in the exit code follows from who can be waiting.
\begin{itemize}
...
\begin{itemize}
  ...
  \item Forgetting to lock or to unlock, or locking and unlocking the wrong
    mutex.  Some compilers and analysis tools can warn about such cases.
...
inside the critical section and signalling afterwards.  Spurious wake-ups
...
described by three multithreading models (\cref{fig:l03-mt-models}).

\begin{figure}[htbp]
  \centering
  \input{figures/l03-mt-models}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l03-boss-workers}
...
a \term{thread pool}[スレッドプール], a set of workers created in advance.
...
requests.  Consider $n$ requests, each needing the same total processing
...
\begin{example}
  ...
  \SI{100}{\milli\second} of work.  The boss-workers design has one boss and
