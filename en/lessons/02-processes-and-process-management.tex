...
\begin{definition}[Process]
  ...
  called a \emph{task} or a \emph{job}.
\end{definition}
...
Besides its execution state and its data, a process may also require
hardware, such as the I/O devices it reads from and writes to.
...
\begin{description}
  ...
  \item[Stack] The \term{stack}[スタック] is a dynamic part of the address
    ...
    order.  When the process calls a procedure, the state needed to resume
    the caller, the return address and saved register values, is pushed
    onto the stack, and the local variables of the called procedure are
    allocated there; when the procedure returns, all of it is popped again.
    The stack starts at the high end of
    the address space and grows toward lower addresses.
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l02-address-space}
...
\begin{example}
  ...
  \texttt{0x1F3A0A10} happens without its involvement.\tip{Translation
  works on whole pages, \SI{4}{\kibi\byte} on x86, so the low 12 bits of an
  address pass through it unchanged: both addresses here end in
  \texttt{A10}.}
\end{example}
...
large portions, such as the region between the heap and the stack, are not
allocated at any given time.  Second, there may not be enough physical
memory for all the state that is allocated.  With 32-bit virtual addresses a single process
can address $2^{32}$ bytes, that is \SI{4}{\gibi\byte}, and several processes
together easily need more memory than the machine has.\margin{On x86-64 the
hardware translates 48-bit addresses and Linux gives each process the lower
half of that range, \SI{128}{\tebi\byte}; the five-level paging of recent
processors extends the addresses to 57 bits (Linux \emph{5-level paging}
documentation).}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l02-address-mapping}
...
times:\caution{Linux has no separate per-process structure: one
\texttt{struct task\_struct} describes one \emph{thread} (Lesson~3), and a
process is a group of tasks that share an address space.}
\begin{itemize}
  ...
  \item Other fields change far too often to be kept current in the PCB.
    ...
    process stops running on the CPU.\margin{\texttt{/proc/PID/status}
    prints many of these fields: state, parent, number of threads, memory
    limits, signal masks.  On x86-64 each task also owns a
    \SI{16}{\kibi\byte} kernel stack (Linux \emph{Kernel Stacks}
    documentation).}
\end{itemize}

\begin{figure}[htbp]
  \centering
  \input{figures/l02-pcb}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l02-context-switch}
...
\begin{description}
  ...
  \item[Indirect cost] the effect on the processor cache.  A cache access
    ...
    takes on the order of hundreds of cycles.\margin{Intel Skylake:
    L1 hit 4--5 cycles, L2 about 12, L3 about 44, DRAM some
    \SI{50}{\nano\second} more (7-cpu.com).}
\end{description}
...
\begin{note}
  The operating system switches contexts regardless of the state of the
  cache, for instance when a process of higher priority becomes ready or
  when the running process has used up the time it was allotted
  (\cref{sec:l02-scheduler}).  The cold cache that the switch leaves behind
  is part of the price of sharing the CPU.
\end{note}
...
\begin{keypoint}
  ...
  it leaves behind, can far exceed the cost of the save and
  restore.\margin{Li et al.\ \cite{li2007} measure
  \SI{3.8}{\micro\second} per switch between processes that touch no data,
  and more than \SI{1000}{\micro\second} in total once their data no longer
  fits in the cache.}
\end{keypoint}
...
\begin{description}
  ...
  \item[Waiting] A process in the \term{waiting state}[待ち状態] cannot
    ...
    operation.\caution{An event completion makes a waiting process ready,
    not running; the CPU is given to it by a dispatch, possibly an
    immediate one.}  When the event occurs, the process becomes ready
    again.
  \item[Terminated] A process that has finished all its operations, or that
    has detected an error, calls \emph{exit} and returns an exit code that
    indicates success or an error; a process that causes a fault, such as
    an invalid memory access, is terminated by the operating system.
    Either way the process enters the
    \term{terminated state}[終了状態].\caution{Linux does not separate
    ready from running: both are \texttt{TASK\_RUNNING}, which \texttt{ps}
    prints as R\@.  S and D are the two kinds of waiting, interruptible and
    uninterruptible, and T is a stopped process.}
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l02-states}
  \caption{Process states and the transitions between them.}
  \label{fig:l02-states}
\end{figure}

\Cref{tab:l02-trace} traces one process through its life
cycle.  Note that
...
\begin{table}[htbp]
  \centering
  \small
  \caption{State trace of a process that runs, is interrupted once, reads a
    file, and exits.  The queues it waits in and the dispatch that takes it
    out of the ready queue are described in \cref{sec:l02-scheduler}.}
  \label{tab:l02-trace}
  \begin{tabular}{@{}llll@{}}
    \toprule
...
shell process is created for the user, and a program started from the
...
has a parent, the processes of a system form a tree
(\cref{fig:l02-creation}\,(a)); on Unix-based systems its root is
\term[init]{\texttt{init}}[init], the first user process that the kernel
starts after booting, and every other user process descends from it.  (On
many current Linux distributions the program that runs as this first
process is \texttt{systemd}.)

\begin{figure}[htbp]
  \centering
  \input{figures/l02-creation}
...
\begin{description}
  \item[\texttt{fork}] With the \term*{\texttt{fork}}[fork]\index{fork@\texttt{fork}} system
    ...
    address space.\caution{\texttt{fork} copies page tables, not memory:
    a page is duplicated only when one side writes to it (copy-on-write,
    Lesson~8).}  The child is a duplicate of the parent, with the same
  ...
  \item[\texttt{exec}] With the \term[exec]{\texttt{exec}}[exec] system
    ...
    operating system replaces the address space of the process with the new
    program and updates the PCB to describe it: the program counter now
    points to the first instruction of the new program, while the process
    identifier and the open files are kept.
\end{description}
...
Android uses \texttt{fork} to start applications quickly.  Its application
processes descend from \term{Zygote}[Zygote], a daemon started during boot
whose sole purpose is to launch applications; it loads the runtime and the
commonly used libraries once.\sidenote{Because \texttt{fork} shares the
pages of the parent copy-on-write, the runtime and the preloaded classes
are not copied into each application: every application process starts with
them already mapped, and they remain in one set of physical pages for as
long as no one writes to them.  Zygote therefore saves start-up time and
memory at once.}
...
\end{lstlisting}
  ...
  \texttt{ls}; the call never returns.\margin{A process that has exited
  keeps its PCB until its parent reads the exit code---here with
  \texttt{waitpid}; until then it is a \emph{zombie} (state Z\@).  A child
  whose parent exits first is adopted and reaped by \texttt{init}.}
\end{example}
...
\begin{example}
  ...
  \SI{20}{\milli\second}, $20/20.5 \approx \SI{97.6}{\percent}$.
\end{example}

A long timeslice is not free, however: while one process holds the CPU, the
other ready processes wait for their turn.  Choosing a scheduler thus
requires design decisions: what timeslice values are appropriate, and which
metrics decide the process to run next.  Lesson~7 examines these
questions.\margin{Real values: Linux CFS shared a \SI{6}{\milli\second}
period among the runnable tasks, with a floor of \SI{0.75}{\milli\second}
each; Linux~6.6 EEVDF uses a base slice of \SI{0.7}{\milli\second}.}
...
it may be dispatched to the CPU right away.\margin{The wait is long
compared with everything else in this chapter: a \SI{4}{\kibi\byte} read
takes tens of microseconds from an NVMe SSD and about
\SI{10}{\milli\second} from a rotating disk.}

\Cref{fig:l02-queues} shows the ways in which a process enters, or
re-enters, the ready queue:
\begin{itemize}
  \item its I/O request has been served by the device;
  \item its timeslice has expired;
  \item a new process has been created by a \texttt{fork}: the child is
    placed on the ready queue;
  \item it was waiting for an interrupt, and the interrupt has
    occurred.\margin{Handling the interrupt is not itself a context
    switch: the handler runs in the kernel, in the context of whichever
    process was interrupted (Lesson~5).}
\end{itemize}

\begin{figure}[htbp]
  \centering
  \input{figures/l02-queues}
  \caption{Ways in which processes enter the ready queue; every path ends
    there.  On the \texttt{fork} path it is the new child, not the process
    that issued the call, that joins the queue.}
  \label{fig:l02-queues}
\end{figure}

A process on an I/O queue is outside the control of the scheduler until the
device has completed its request; the scheduler chooses only among the
processes in the ready queue, and decides when to switch contexts and to
which process.

\Needspace{7\baselineskip}
\begin{keypoint}
  The scheduler preempts the running process, and chooses and dispatches
  the next one from the ready queue.  Because it runs on the same CPU, its
  own time is overhead (\cref{eq:l02-useful}), which shrinks as the
  timeslice grows relative to $t_{\mathrm{sched}}$.
\end{keypoint}
...
Shared memory moves the cost of communication from the individual exchanges
to the setup.\margin{Measured on Linux, a round trip through a pipe costs
on the order of \SI{10}{\micro\second} and one through a Unix-domain socket
somewhat more (lmbench \texttt{lat\_pipe}, \texttt{lat\_unix}); a minimal
system call alone costs some \SI{100}{\nano\second}.}  Once the region is
mapped, an exchange involves no copy and no crossing into the kernel, but
creating the mapping is itself costly, so which of the two mechanisms is
cheaper follows from how many exchanges the processes make.
...
\begin{example}
  ...
  cheaper only when $300 + n < 6n$, that is, for more than 60
  messages.\tip{The break-even count is the setup cost divided by the
  saving per exchange, here $300/(6-1) = 60$ messages.}
\end{example}
...
\begin{keypoint}[Lesson summary]
  ...
  \texttt{exec}; the CPU scheduler picks the next ready process, and its
  own overhead depends on the timeslice; and IPC, by message passing or
  shared memory, lets isolated processes communicate.
\end{keypoint}
