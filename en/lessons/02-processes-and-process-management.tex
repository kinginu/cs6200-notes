...
cycle.\caution{Linux does not separate ready from running: both are
\texttt{TASK\_RUNNING}, which \texttt{ps} prints as R\@.  S and D are the
two kinds of waiting, interruptible and uninterruptible, and T is a stopped
process.}  Note that
...
to the setup.  Once the region is mapped, an exchange involves no copy and
no crossing into the kernel, but creating the mapping is itself costly, so
which of the two mechanisms is cheaper follows from how many exchanges the
processes make.\margin{Measured on Linux, a round trip through a pipe costs
