...
Pthreads was first standardized in 1995 as the threads extension POSIX.1c
and is now part of the base POSIX.1 standard.  Like the rest of POSIX, it
...
models of Lesson~3.  All Pthreads names begin with \texttt{pthread\_}; the
...
other information needed to manage it.  Threads are created and joined with
two functions:
...
default attributes.  An attribute object is itself opaque and is manipulated
...
further threads, as soon as the thread exists.  \Cref{tab:l04-attributes}
lists the attributes that matter in this course.  The scope attribute
...
By default a Pthreads thread is a \term{joinable thread}[ジョイン可能スレッド]:
another thread can join it, and when it terminates, its identifier and exit
status are kept until a \texttt{pthread\_join} collects them.\source{P2L3,
detaching PThreads; Kerrisk ch.~29.}  With joinable threads, a parent thread
creates children and joins them later.  The parent should not terminate
before its children have completed and have been joined, for two reasons.
A child that has terminated but has not been joined continues to occupy
resources: it remains a \emph{zombie} thread until it is joined, and if no
thread ever joins it, these resources are never reclaimed while the process
lives.  And if the
parent is the main thread and returns from \texttt{main}, the whole process
terminates, together with every child that has not yet finished.
...
\end{lstlisting}
A detached thread cannot be made joinable again.  Since nobody waits for
it, a parent can exit while its detached children continue to execute
(\cref{fig:l04-join-detach}).  Parent and children are then equals, except
that the parent holds the identifiers of the children it created.  To exit
without ending the process, the parent calls \texttt{pthread\_exit(status)}:
it terminates only the calling thread, and the process continues until its
last thread has terminated.\caution{Returning from \texttt{main}, or
calling \texttt{exit} in any thread, terminates the whole process, including
detached threads that have not finished.}

\begin{figure}[tbp]
  \centering
  \input{figures/l04-join-detach}
  \caption{Left: a joinable child that terminates before it is joined keeps
    ...
    a detached child continues after the main thread has called
    \texttt{pthread\_exit}, and releases its resources itself.}
...
\begin{enumerate}
  ...
  \item \textbf{The compiler flag.}  The program is compiled and linked
    with \texttt{-pthread} or \texttt{-lpthread}:
...
\end{lstlisting}
    ...
    choice.\margin{Since version~2.34 the GNU C library contains the
    Pthreads functions in \texttt{libc} itself, so on current Linux systems
    linking succeeds even without either flag.}
  \item \textbf{Checked return values.}  Pthreads functions report failure
    ...
    otherwise, rather than through \texttt{errno}.  A program that ignores the
    result of \texttt{pthread\_create} may later join a thread that was
    never created, using an identifier that was never set.
\end{enumerate}

\Needspace{8\baselineskip}
\begin{example}
  Checking the result of thread creation:
  \begin{lstlisting}[language=C, numbers=none]
...
if (rc != 0) {
  ...
  \end{lstlisting}
  \texttt{strerror} (from \texttt{<string.h>}) turns the error number into
  a message.
\end{example}
...
joins them.  Each worker prints one line.  The workers share no data, so
...
the index through its argument:
\begin{lstlisting}[language=C, numbers=none]
...
\end{lstlisting}
The program does not reliably print $0$, $1$ and $2$.  Every thread receives
...
\begin{table}[htbp]
  ...
  \caption{One execution of the program that passes \texttt{\&i} with three
    threads.  T0 and T1 read \texttt{i} after \texttt{main} has changed it,
    and T2 reads it after the join loop has reset it; the program prints
    1, 1 and 0.}
...
belongs to it alone and that no other thread modifies:
\begin{lstlisting}[language=C, numbers=none]
...
occur.  The array lives in the stack frame of \texttt{main}, which does not
...
thread blocks until the mutex is released.  The difference is syntactic, but
...
Three further operations complete the basic mutex interface:
\begin{lstlisting}[language=C, numbers=none]
...
\begin{description}
  \item[\texttt{pthread\_mutex\_init}] initializes a mutex before its first
    ...
    shared among processes (Lesson~9); \texttt{NULL} selects the defaults.
...
\begin{example}
  ...
  free, rather than delaying its main work:
  \begin{lstlisting}[language=C, numbers=none]
...
\texttt{Wait(mutex, cond)}: \texttt{pthread\_cond\_wait} takes the condition
variable first.}

POSIX allows \texttt{pthread\_cond\_wait} to return even though no thread
has signalled; the standard itself calls such returns spurious
wake-ups.\index{spurious wake-up}  A woken thread must therefore test its
predicate again, and the \texttt{while} loop of Lesson~3 is not merely good
practice but required:
\begin{lstlisting}[language=C, numbers=none]
...
\texttt{PTHREAD\_COND\_INITIALIZER} also provides.
...
\begin{itemize}
  ...
  \item \textbf{When in doubt, broadcast.}  Broadcasting where a signal
    ...
    since these threads are woken up and scheduled for nothing.
  \item \textbf{A mutex is not needed to signal or broadcast.}  The
    notification can be issued after the mutex has been released, which
    avoids the spurious wake-ups of Lesson~3: the woken threads do not have
    to wait for a mutex that the signalling thread still holds.
\end{itemize}
...
\begin{keypoint}
  ...
  the predicate may have changed, and preferably notify after unlocking.
\end{keypoint}
...
number \texttt{num} of values in the buffer.  Both indices advance modulo
...
\begin{figure}[htbp]
  \centering
  \input{figures/l04-bounded-buffer}
...
The producer inserts the values $1,\dots,\texttt{NUM\_ITEMS}$:
\begin{lstlisting}[language=C, numbers=none]
void *producer(void *param) {
  int i;
...
\begin{itemize}
  ...
  \item Each thread signals after unlocking, so the woken thread does not
    have to wait for \texttt{m}.  A signal when nobody waits has no effect;
  ...
  \item Because producers wait only on \texttt{c\_prod} and consumers only
    ...
    that can use it.  A signal therefore suffices even with several
    producers and several consumers: each insertion enables exactly one
    removal, and each removal exactly one insertion.
...
\begin{keypoint}[Lesson summary]
  ...
  \texttt{while} loop and notified, preferably after unlocking, whenever a
  predicate may have changed, as in the producer-consumer program.
\end{keypoint}
