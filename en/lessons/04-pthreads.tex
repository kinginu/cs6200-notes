...
models of Lesson~3.  The names of its functions and types begin with
\texttt{pthread\_} and those of its constants with \texttt{PTHREAD\_}; the
...
resources for as long as the process lives.  A main thread must in addition
...
\end{lstlisting}
    ...
    choice.\margin{Since glibc~2.34 the Pthreads functions are part of
    \texttt{libc} itself.}
  \item \textbf{Checked return values.}  Pthreads functions report failure
    ...
    otherwise, rather than through \texttt{errno}; a caller that cannot
...
loop of Lesson~3 is not merely good practice but required:
\begin{lstlisting}[language=C, numbers=none]
