...
OSTEP ch.~6; Silberschatz et al.\ ch.~2.}  The process runs in user mode
...
int main(void) {
  ...
  expects them and executes the trap.  The output channel~1 and the length
  are passed directly; the text is passed indirectly, through its address
  \texttt{msg}.\margin{On x86-64 Linux the number is placed in register
  \texttt{rax}, the arguments in \texttt{rdi}, \texttt{rsi} and
  \texttt{rdx}, and the \texttt{syscall} instruction performs the trap.}
\end{example}
...
looks like an ordinary procedure call in C, but it costs far more: a mode
switch, a change of execution context and the loss of cache locality.}
...
code bases, which makes them a source of bugs.\margin{Linux modules run in
kernel mode in the kernel's address space; Silberschatz et al.\ therefore
...
A microkernel is small, which lowers its overhead and can improve
performance for some applications, and its code is easier to verify.  This
makes microkernels attractive for embedded devices and control systems,
where correct behaviour of the operating system is critical.  On the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}>{\RaggedRight}p{19mm}>{\RaggedRight}p{46mm}%
    ...
    Modular & maintainable; smaller footprint; fewer resources needed
      ...
      modules \\
    \addlinespace
    Microkernel & small size and low overhead; easier to verify
...
(\cref{fig:l01-macos}).\margin{Mach and BSD execute in the same kernel
address space, so Silberschatz et al.\ describe the result as a hybrid of
the microkernel and monolithic organizations.}  Mach implements the key
...
\begin{figure}[htbp]
  ...
  \caption{The structure of Mac OS X: the Mach microkernel with a BSD
    layer, the I/O Kit and kernel extensions.}
  \label{fig:l01-macos}
\end{figure}
