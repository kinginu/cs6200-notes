...
overseeing and controlling how applications share it.\margin{Latencies the
abstraction hides: L1 hit \SI{1}{\nano\second}, DRAM
\SI{80}{\nano\second}, SSD read \SI{50}{\micro\second}, disk seek
\SI{5}{\milli\second}.  OSTEP ch.~36--37.}  The definition
names three roles.
\begin{description}
...
fixed-size block of memory.\margin{On x86-64 a page is \SI{4}{\kibi\byte} by
default; huge pages of \SI{2}{\mebi\byte} and \SI{1}{\gibi\byte} also exist.
Lesson~8.}  The mechanisms allocate a page and map it into
...
\begin{description}
  ...
  \item[Signals] travel in the opposite direction.  A
    ...
    has set has expired.\margin{A signal is delivered when the process next
      returns to user mode, not at an arbitrary instruction.  Signals:
      Lesson~5.}
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l01-user-kernel}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l01-syscall-flow}
...
asynchronous system calls are discussed in later lessons.\margin{Not every
call crosses: Linux maps the vDSO into every process, so
\texttt{clock\_gettime} runs in user mode.  \texttt{vdso(7)}.}
...
\begin{itemize}
  ...
  \item They require executing a number of instructions to save and
    ...
    transition takes about \SIrange{50}{100}{\nano\second}.\sidenote{On a
      Core~i7, entering and leaving the kernel cost \num{79} and \num{71}
      cycles, but a \texttt{pwrite} also evicts half of the
      \SI{32}{\kibi\byte} L1 data cache, and the application needs up to
      \num{14000} cycles to recover \cite{soares2010}.}
...
Applications reach all of these services through system calls.\margin{Linux on
x86-64 numbers some \num{390} system calls (\texttt{syscall\_64.tbl}); the C
library wraps them in a much larger API.}  Different
...
maintain and upgrade, and leaves more resources to applications.\margin{The
Linux source passed \num{40} million lines in 2025, some \SI{60}{\percent} of
it drivers; a kernel loads only a small part.}  The
...
crossings of the user/kernel boundary are costly.\sidenote{How costly is a
matter of design: Liedtke's L3/L4 \cite{liedtke1993} transferred a short
message between address spaces in \SI{5}{\micro\second} on a
\SI{50}{\mega\hertz} i486, where Mach needed about \SI{115}{\micro\second} on
the same processor; seL4 reports a few hundred cycles per one-way IPC today.}
...
\texttt{fork};\caution{\texttt{printf} and \texttt{malloc} are library
functions, not system calls: they call \texttt{write} and \texttt{mmap}
only when they must.} the standard utility programs, such as the shell,
editors and compilers, use the library; and users interact with the utility
...
interface.\tip{\texttt{strace ./prog} lists the system calls a program
actually makes; \texttt{ltrace} does the same for library calls.}

\begin{figure}[htbp]
  \centering
  \input{figures/l01-linux}
...
contains the dispatcher, which switches the CPU between
processes.\margin{The dispatcher is the subject of Lesson~7, virtual memory
and the page cache of Lesson~8, and signals of Lesson~5.}  In
