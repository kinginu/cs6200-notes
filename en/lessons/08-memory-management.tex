...
The translation itself is performed entirely in hardware.\caution{Not every
architecture translates in hardware alone: on MIPS and SPARC a TLB miss
raises an exception and a kernel handler fills the TLB in software.  OSTEP
ch.~19.}  The operating system creates and maintains the data structures
that the hardware uses, such as the page tables, but the hardware dictates
their format and therefore which memory management mechanisms are possible:
for example, the page sizes and the layout of a page table entry.

\begin{figure}[htbp]
  \centering
  \input{figures/l08-mmu}
...
(PFN) of the frame that contains the page.\caution{A PFN need not be as wide
as a VPN.  With physical address extension (PAE), 32-bit x86 maps 20-bit
VPNs onto 24-bit PFNs and so addresses \SI{64}{\gibi\byte} of physical
memory.}  The physical address combines the PFN with the unchanged offset
(\cref{fig:l08-translation}):
\begin{equation}
...
page table covers $2^{k+d}$ bytes of the virtual address space.\tip{A table
of $2^{9}$ entries of 8 bytes is exactly \SI{4}{\kibi\byte}: an index of 9
bits per level makes every table fit in one page frame.}  A gap in
...
make each translation slower.\margin{x86-64 implements only 48 of the 64
virtual address bits.  Intel's 5-level paging adds a fifth level and raises
this to 57 bits (\SI{128}{\pebi\byte}); Linux has supported it since~4.14.}
...
so that the access can be checked without the page table.\margin{The MMU
also caches entries of the upper levels (paging-structure caches), so a miss
usually costs fewer than $L$ accesses.  Intel SDM vol.~3A, 4.10.3.}  A small
number of entries suffices for a high hit rate because memory accesses
exhibit \emph{locality}: a page that has just been accessed is likely to be
accessed again soon (temporal locality), and addresses close to it, which
usually lie in the same page, are likely to be accessed next (spatial
locality).  An Intel Core i7 processor of the Sandy Bridge generation, for
example, has per core a data TLB of 64 entries, an instruction TLB of 128
entries, and a second-level TLB of 512 entries shared by both kinds of
translations (all for \SI{4}{\kibi\byte} pages).\margin{A context switch
makes TLB entries stale; they are flushed unless tagged with their address
space (Lesson~12).}
...
\begin{example}
  ...
  $t_{\mathrm{TLB}} = \SI{1}{\nano\second}$ and $L =
  4$.\tip{\Cref{eq:l08-eat} rearranges to $t_{\mathrm{eff}} =
  t_{\mathrm{TLB}} + \bigl(1 + L(1-h)\bigr)\,t_{\mathrm{mem}}$: the lookup
  is always paid, the walk only on a miss.}  Without a TLB, every access
  costs $5 \cdot 80 = \SI{400}{\nano\second}$.  With a hit rate of
  \SI{99}{\percent}, \cref{eq:l08-eat} gives $0.99 \cdot 81 + 0.01 \cdot 401
...
size of their address spaces.\caution{One entry per frame means one
$(\text{pid}, p)$ pair per frame: a frame that several processes share does
not fit the structure, which makes shared memory awkward.}  A virtual
address consists of the process identifier (pid), the page number $p$ and
the offset $d$.  To translate it, the table is searched for the entry that
matches $(\text{pid}, p)$; the index $i$ of that entry is the frame number,
and the physical address is formed from $i$ and $d$
(\cref{fig:l08-inverted}\,(a)).

\begin{figure}[htbp]
  \centering
  \input{figures/l08-inverted}
...
next element, so a translation searches only one short list.\margin{Hashed
inverted tables were used by the IBM AS/400, by 64-bit PowerPC and by
Itanium; x86 has always used multi-level tables.}
...
the permissions of the segment.\margin{A descriptor is 8 bytes; a selector
holds a 13-bit index---hence the 8192 entries---a bit choosing GDT or LDT,
and a 2-bit privilege level.}  The sum of base and offset is a \emph{linear
address}.  Segmentation is combined with paging: the linear address is
translated into a physical address by the page tables
...
\begin{itemize}
  ...
  \item On x86-64, segmentation is retained only for backward
    ...
    translates addresses.\sidenote{The two exceptions are in use.  Linux
    keeps the thread-local storage of the running thread at the FS base and
    the per-CPU data of the kernel at the GS base; the instruction
    \texttt{swapgs} exchanges the user and the kernel value of the latter
    when a system call enters the kernel.}
\end{itemize}

\begin{figure}[htbp]
  \centering
  \input{figures/l08-segmentation}
...
\begin{description}
  ...
  \item[User-level allocators] manage the dynamic state of a process, its
    ...
    again.  Examples are dlmalloc, jemalloc, Hoard and
    tcmalloc.\margin{glibc's \texttt{malloc} grows the heap with
    \texttt{brk}, but serves each request above the mmap threshold,
    \SI{128}{\kibi\byte} by default, with an \texttt{mmap} of its own.}
\end{description}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l08-fragmentation}
...
two, however, causes internal fragmentation.\margin{Linux keeps free lists
for the orders 0--10, i.e.\ 1 to 1024 frames or \SI{4}{\kibi\byte} to
\SI{4}{\mebi\byte}; \texttt{/proc/buddyinfo} counts the free blocks of each
order.}
...
therefore allocates its objects with a slab allocator
\cite{bonwick1994}.\sidenote{Bonwick wrote the first slab allocator for
Solaris~2.4.  Linux has had three implementations of the idea---SLAB, SLUB
and the compact SLOB---of which SLUB has been the default since~2.6.23;
\texttt{/proc/slabinfo} lists the caches and their object sizes.}
...
underlying slabs are obtained from the buddy allocator.\margin{Each cache
also keeps per-CPU lists of free slots, so that the common case of an
allocation touches no shared state and takes no lock (Lesson~10).}

\begin{figure}[htbp]
  \centering
  \input{figures/l08-slab}
...
\begin{enumerate}
  ...
  \item It reads the page from the swap partition into a free page frame,
    using an I/O operation.\margin{The \SI{8}{\milli\second} of the example
    below is a rotating disk.  An NVMe SSD reads a page in tens of
    microseconds, which makes a fault rate two orders of magnitude higher
    tolerable.}
...
\begin{example}
  ...
  of accesses faulting, the average access costs $(1-p) \cdot 100 + p \cdot 8 \cdot 10^{6}$
...
that it swaps out.  This choice is called \term{page
replacement}[ページ置換], and it raises two questions: when pages should be
swapped out, and which ones.
...
\end{itemize}
Linux provides parameters that tune when reclaim starts and
stops,\caution{In Linux both watermarks are thresholds on free memory:
reclaim starts below the low and stops above the high one.} categorizes
pages into types, for instance reclaimable and swappable pages, and uses a
variation of LRU that gives pages a second chance.
...
entire address space of its parent (Lesson~2).\margin{The page tables are
copied in any case: \SI{1}{\gibi\byte} of \SI{4}{\kibi\byte} pages needs
$2^{18}$ entries of 8 bytes, \SI{2}{\mebi\byte}.}  Many of its pages, such as
...
actually modified.\caution{The saving is not unconditional.  Every first
write traps into the kernel, on the order of a microsecond, so a child that
writes almost all of its pages is slower than one whose address space was
copied eagerly.}

\begin{figure}[htbp]
  \centering
  \input{figures/l08-cow}
...
\begin{description}
  ...
  \item[Migration] With \term{process migration}[プロセスマイグレーション]
    ...
    consolidation of processes onto fewer machines.\margin{Clark et al.\ \cite{clark2005}
    migrated running Xen virtual machines with \SI{210}{\milli\second} of
    downtime for a web server and \SI{60}{\milli\second} for a game server.}
\end{description}
...
process dirties pages as fast as they are copied.\margin{This loop is called
\emph{pre-copy} migration.  \emph{Post-copy} does the opposite: it resumes
the process at the destination at once and faults its pages across the
network.}
