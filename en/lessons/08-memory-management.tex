...
\begin{example}
  ...
  $2^{36}$ entries and $2^{39}$ bytes, \SI{512}{\gibi\byte}, per process.  At
  the other extreme, a microcontroller with 16-bit addresses needs $2^{6} =
  64$ entries with \SI{1}{\kibi\byte} pages, and $2^{8} = 256$ entries with
  pages of 256 bytes.
\end{example}
...
directory entry is simply marked invalid.
...
for \SI{4}{\kibi\byte} pages).\margin{After a context switch the TLB entries
of the previous process are stale; they are flushed unless the entries are
tagged with their address space (Lesson~12).}
...
internal fragmentation, and external fragmentation is not an issue: a slot
that is freed fits the next object of the same type
(\cref{fig:l08-slab}).  The underlying slabs are obtained from the buddy
allocator.
...
\begin{example}
  ...
  that reads the page from disk costs \SI{8}{\milli\second}, i.e., $8 \cdot
  10^{6}$ times as much.  With a fraction $p$ of accesses faulting, the
...
\end{itemize}
Linux provides parameters that tune these thresholds, categorizes pages
into types, for instance reclaimable and swappable pages, and uses a
...
\begin{example}
  ...
  A survives the second scan because the process referenced it again after
  its bit was cleared, whereas C, whose bit was cleared and which has not
  been referenced since, would be the next victim.
\end{example}
...
\begin{example}
  A process with an address space of 25\,000 pages
  (about \SI{100}{\mebi\byte}) forks, and the child writes to 40 pages of
  its stack and data before it calls \texttt{exec}.  Copy-on-write copies
  40 pages instead of 25\,000, i.e., \SI{0.16}{\percent} of the address
  space.
\end{example}
