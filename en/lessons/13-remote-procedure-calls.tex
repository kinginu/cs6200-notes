...
processes run on different machines connected by a network.\margin{Light
in fibre covers about \SI{200}{\kilo\meter\per\milli\second}, so a call to a
machine \SI{5000}{\kilo\meter} away cannot return in under
\SI{50}{\milli\second}, however fast the two machines are.}  Across
...
RPC systems, including the two examined in this lesson.\sidenote{Their
measurements, between two Dorado workstations on a
\SI{2.94}{\mega\bit\per\second} Ethernet, give the scale of the overhead: a
call with no arguments and no results took \SI{1097}{\micro\second} in the
median, of which \SI{131}{\micro\second} was transmission, against
\SI{9}{\micro\second} for the same call made locally \cite{birrell1984}.}
...
\begin{description}
  ...
  \item[4 Receive] The RPC runtime of the server receives the message and
    ...
    this point.\caution{Sun RPC's default credentials, \texttt{AUTH\_SYS},
    are no more than the user and group identifiers that the client claims;
    real authentication needs \texttt{RPCSEC\_GSS} \cite{rfc2203}.}
...
\begin{definition}[Unmarshalling]
  ...
  needed.\caution{Marshalling and serialization are interchangeable here.
  Where they are distinguished, serialization is the byte layout alone and
  marshalling adds what the receiver needs to rebuild the value, such as the
  code base of a class in Java.}
\end{definition}
...
\begin{example}
  ...
  \texttt{count} $=3$ and \texttt{values} $=\mathtt{0x9c40}$.\margin{The
  data are copied at least twice on the way, into the send buffer and out of
  the receive buffer; the shared memory of Lesson~9 exists to avoid exactly
  these copies.}
\end{example}
...
the request when it expires, but timeouts and retries give no
guarantees.\sidenote{Retrying until a reply arrives gives
\emph{at-least-once} semantics and giving up after the first attempt
\emph{at-most-once}; neither is exactly-once.  A server comes close by
keeping a cache of recent request identifiers together with their replies
and answering a retransmission from the cache instead of executing the
procedure again---NFS servers call it the duplicate request cache.}
...
service uniquely.\margin{Numbers \texttt{0x00000001}--\texttt{0x1fffffff}
are assigned by IANA---NFS is \texttt{100003}---and
\texttt{0x20000000}--\texttt{0x3fffffff} are left to local administrators,
which is where \texttt{0x20000123} comes from \cite{rfc5531}.}  A client
that sets
...
\texttt{sum\_proc\_1} (\cref{sec:l13-sun-binding}).\tip{The generated names
follow a rule: the procedure name in lower case, then the version
number---\texttt{sum\_proc\_1}---with \texttt{\_svc} added for the procedure
the developer writes.}  Each is compiled and
...
port 111, and the NFS server as program \texttt{100003}.\margin{NFS also
listens on the registered port 2049, and NFSv4 clients are told not to use
the binding protocol at all \cite{rfc8881}.}  The last line is
...
stub.\margin{The server's own port is ephemeral and differs after every
restart, so a firewall cannot open it in advance; \texttt{rpcbind} itself
answers any query, and exposed daemons have been abused to amplify
denial-of-service traffic.}  Besides the
address of the server, the handle holds the
...
over UDP the runtime retransmits the request while it waits.\margin{An RPC
message over UDP must fit in a single datagram, at most \SI{65507}{\byte} of
payload; the runtime of glibc retransmits every \SI{5}{\second} until the
call times out.}
...
becomes a C structure with a length field and a pointer to the
elements.\margin{The maximum is enforced: the generated routine refuses a
longer array, so the call fails instead of overrunning the receiver.}
...
serialized element by element \cite{rfc4506}.\caution{A structure with a
cycle defeats this encoding: the stubs would follow the pointers for ever.
Java's object serialization instead gives every object a handle and writes a
back-reference the second time.}
...
\begin{description}
  \item[Transport header] the headers of the transport protocol, such as
    TCP or UDP, and of the protocols below it.\margin{Over a byte stream
    each message is preceded by a \SI{4}{\byte} record mark: a last-fragment
    flag in the top bit and a 31-bit length \cite{rfc5531}.}
...
\begin{itemize}
  \item Every item is encoded in a multiple of \SI{4}{\byte}; shorter data
    are padded with zero bytes.\tip{An item of $n$ bytes occupies
    $4\lceil n/4\rceil$ on the wire, and a variable-length array or string
    adds a \SI{4}{\byte} length field.}
...
client code beneath it calls the service.\margin{The RMI registry is itself
a remote object; \texttt{LocateRegistry.getRegistry} returns a stub for the
one at TCP port 1099 unless another port is given.}
...
together with the objects they refer to.\caution{The copy is made on every
invocation, and changes that the server makes to it are invisible to the
client---the trap of the serialized pointers of \cref{sec:l13-pointers}.}
Remote objects are passed by
reference, as stubs.
...
\end{description}
\Cref{tab:l13-design} compares the design choices of Sun RPC and Java
RMI.\margin{gRPC makes the same four choices differently: Protocol Buffers
as IDL and encoding, DNS and a load balancer in place of a registry, and a
status code returned by every call.}
