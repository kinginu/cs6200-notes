...
of an integer at the lowest address, and \emph{little-endian}%
\index{little-endian} if it stores the least significant byte there.  The
x86 processors are little-endian; many network protocols, and the encoding
of \cref{sec:l13-encoding}, use big-endian order.
...
parts: user, user stub, RPCRuntime on each side, server stub and server.  A
program called Lupine generated the stubs \cite{birrell1984}.}

\begin{figure}[htbp]
  \centering
  \input{figures/l13-steps}
...
\begin{figure}[htbp]
  ...
  \caption{Three partial failures that the client cannot distinguish.  The
    ...
    boxes the execution of the procedure.  (a)~The request is lost; (b)~the
...
whether the procedure has executed.\margin{Birrell and Nelson guaranteed
that the procedure had executed exactly once if a call returned; otherwise
the call raised an exception, and the procedure might or might not have
executed \cite{birrell1984}.}
...
Sun RPC makes the design choices of \cref{sec:l13-failures} as follows.
\begin{description}
...
\Cref{lst:l13-sum-x} specifies a service that returns the sum of up to 64
integers.

\begin{lstlisting}[language=C, numbers=none, caption={The XDR
  specification \texttt{sum.x}.}, label={lst:l13-sum-x}]
const MAX_VALUES = 64;
...
contains procedures.\margin{By convention procedure~0 of every version is
the null procedure, which takes no arguments and returns nothing; clients
use it to check that a server responds \cite{rfc5531}.}  Program, version
and procedure are identified by numbers, and a request names all three.
The 32-bit program number identifies the service uniquely; numbers from
\texttt{0x20000000} to \texttt{0x3fffffff} are reserved for services
defined locally, such as this one \cite{rfc5531}.  A client that sets
...
implementation of the procedure, \texttt{sum\_proc\_1\_svc}
(\cref{lst:l13-server}); on the client side it is the program that includes
\texttt{sum.h} and calls \texttt{sum\_proc\_1}
(\cref{sec:l13-sun-binding}).  Each is compiled and linked with the
generated files.  The RPC runtime library does the rest: the interaction
with the operating system, such as the use of sockets, and the management
of the communication.

\begin{lstlisting}[language=C, numbers=none, caption={Implementation of the
  procedure, \texttt{server.c}.}, label={lst:l13-server}]
#include "sum.h"
...
The daemon \texttt{rpcbind} replaces the older \emph{portmapper} daemon,
\texttt{portmap}, which offered the same service for TCP and UDP ports
\cite{rfc1833}.  Because port 111 is a privileged port, the daemon must be
started with superuser privileges.  When a server starts, the generated
...
and the port.  The daemon itself appears as program \num{100000} on port
111, and the NFS server as program \num{100003}.  The last line is the
\texttt{sum} server, whose program number \texttt{0x20000123} is printed in
decimal.

\begin{figure}[htbp]
  \centering
  \input{figures/l13-rpcbind}
...
which error messages are derived.  \Cref{lst:l13-client} shows the complete
client.

\begin{lstlisting}[language=C, numbers=none, caption={The client,
  \texttt{client.c}.}, label={lst:l13-client}]
#include <stdio.h>
#include <stdlib.h>
#include "sum.h"
...
\texttt{clnt\_perror} prints it.  The stubs generated by \texttt{rpcgen}
wait up to \SI{25}{\second} for a reply, a timeout that
\texttt{clnt\_control} can change; over UDP the runtime retransmits the
request at shorter intervals within this time.
...
of the interface.  \Cref{lst:l13-xdr} shows the routine for
\texttt{sum\_in} as generated by \texttt{rpcgen}.

\begin{lstlisting}[language=C, numbers=none, caption={Generated XDR routine
  for \texttt{sum\_in}.}, label={lst:l13-xdr}]
bool_t
...
\begin{example}
  ...
  \Cref{tab:l13-encoding} traces the request with which the client of
  \cref{lst:l13-client} calls \texttt{SUM\_PROC} over TCP, with request
  identifier \num{1234} and no authentication.  Over TCP every message is
  preceded by a \SI{4}{\byte} \emph{record mark}, whose highest bit marks
  the last fragment of a message and whose other bits give the length of
  the fragment \cite{rfc5531}.  The RPC header takes \SI{40}{\byte} and the
  argument \SI{16}{\byte}, so the message occupies $4 + 40 + 16 =
  \SI{60}{\byte}$ of TCP payload.  The value $-2$ is encoded as
  \texttt{ff ff ff fe}, and 300 as \texttt{00 00 01 2c} regardless of the
  byte order of the client.  The reply carries the same xid, a header of
  \SI{24}{\byte} and the result 305 in \SI{4}{\byte}.
\end{example}

\begin{table}[htbp]
  \centering
  \caption{The TCP payload of a call to \texttt{SUM\_PROC}.  Rules separate
    the record mark, the RPC call header and the XDR-encoded argument.}
  \label{tab:l13-encoding}
  \small
  \begin{tabular}{@{}rlll@{}}
    \toprule
    Offset & Bytes & Field & Value \\
    \midrule
     0 & \texttt{80 00 00 38} & record mark & last fragment, 56 bytes \\
    \midrule
     4 & \texttt{00 00 04 d2} & xid & 1234 \\
     8 & \texttt{00 00 00 00} & message type & 0 (call) \\
    12 & \texttt{00 00 00 02} & RPC version & 2 \\
    16 & \texttt{20 00 01 23} & program & \texttt{SUM\_PROG} \\
    20 & \texttt{00 00 00 01} & version & \texttt{SUM\_VERS} $=1$ \\
    24 & \texttt{00 00 00 01} & procedure & \texttt{SUM\_PROC} $=1$ \\
    28 & \texttt{00 00 00 00} & credentials: flavor & none \\
    32 & \texttt{00 00 00 00} & credentials: length & 0 \\
    36 & \texttt{00 00 00 00} & verifier: flavor & none \\
    40 & \texttt{00 00 00 00} & verifier: length & 0 \\
    \midrule
    44 & \texttt{00 00 00 03} & \texttt{values}: length & 3 \\
    48 & \texttt{00 00 00 07} & \texttt{values[0]} & 7 \\
    52 & \texttt{ff ff ff fe} & \texttt{values[1]} & $-2$ \\
    56 & \texttt{00 00 01 2c} & \texttt{values[2]} & 300 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\Cref{lst:l13-rmi} declares the \texttt{sum} service as a remote interface
and calls it.

\begin{lstlisting}[language=Java, numbers=none, caption={A remote interface
  and its use in Java RMI.}, label={lst:l13-rmi}]
...
array in \cref{lst:l13-rmi}, are passed by value: they are serialized
together with the objects they refer to.  Remote objects are passed by
reference, as stubs.

\begin{figure}[htbp]
  \centering
  \input{figures/l13-rmi}
  \caption{Java RMI.  Invocations pass from the client through the stub,
    ...
    they are delivered to the remote object.}
  \label{fig:l13-rmi}
\end{figure}

The RMI runtime consists of two layers beneath the stubs.
\begin{description}
